...
/**
 * 2026-09-08：代理商 Action 客户端已删 —— (agent) route group 完全移除后没有 caller。
 * 如未来要恢复 ToB 业务，重新定义 agentAction 时记得在 schema.ts 的 user.agentId
 * 字段同时启用，并把 (agent) route group + checkAgent middleware 加回 proxy.ts。
 */
