...
/**
 * /image-gen —— 公开 demo 生图页面
 *
 * 2026-09-09：代理商概念已砍掉，/image-gen 回到原公开 demo 设计：
 *   - 无登录要求（任何人匿名访问）
 *   - 选效果（effect）→ 可选上传参考图 → 生成图片 → 看结果 / 下载
 *   - 生成记录仅存浏览器 localStorage（key=mooncoda_public_imagegen_history，最多 30 条）
 *   - 单任务轮询：刷新页面能从 localStorage 恢复进行中的任务
 *
 * API 复用：
 *   - GET  /api/public/generate     拿 active 效果列表
 *   - POST /api/public/upload       R2 预签名直传参考图
 *   - POST /api/public/generate     发起生图（同步返回 or 返 taskId）
 *   - GET  /api/image/task/[id]     轮询异步任务
 *
 * 与 workbench 路线（已删）的区别：本页是"一次性体验"——选效果、出图、下载，
 * 不创建 promptOrder，不扣 credit，不接订单系统。
 * 业务方真正下单走 /sign-in 后从工作台 V1 / 公共订单入口进入。
 */
                    ...
                    >
                      <div className="absolute inset-0 flex items-center justify-center p-1.5">
                        {m.previewUrl ? (
                          ...
                          // 无封面：渐变 + 名称首字 placeholder（2026-09-09 同步自
                          // prompt_template 后很多模板没封面图，不能破图）
