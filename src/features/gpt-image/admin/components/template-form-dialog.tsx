 ...
 * - shadcn Dialog/Input/Label/Select/Switch/Textarea/Button → antd
 * - sonner toast → antd App.useApp().message
 * - 用 antd Form.Item 包裹 label + input；Switch antd 原生；Textarea 用 Input.TextArea
 */
...
export function TemplateFormDialog({
  ...
  const handleSave = async () => {
    if (!name.trim() || !description.trim() || !prompt.trim()) {
      message.error("请填写模板名称、用户描述和提示词");
      return;
    }
  ...
  return (
    ...
    >
      <p className="text-sm text-muted-foreground mb-4">
        ...
        <code className="mx-1 rounded bg-muted px-1 text-xs">{"{{变量}}"}</code>
        会在 image-gen 工作台让用户填值后替换。
      </p>

      <Form layout="vertical" className="space-y-3">
        <Form.Item label="模板名称" required className="!mb-0">
          <Input
            value={name}
        ...
        <Form.Item label="用户可见描述" required className="!mb-0">
          <Input.TextArea
            value={description}
        ...
        <Form.Item label="提示词内容" required className="!mb-0">
          <Input.TextArea
            value={prompt}
          ...
          />
          <p className="text-xs text-muted-foreground mt-1">
            用户端<span className="font-medium text-rose-600">不会</span>
            看到此内容。gpt-image 后端会用此提示词 + 用户上传的图片调用
            GPT-Image-2 编辑接口生成效果图；image-gen 工作台会先替换 {"{{key}}"}{" "}
            再调用其他生图模型。
          </p>
        </Form.Item>
        ...
        <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm">提示词变量（image-gen 工作台用）</span>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Form.Item label="输出图片尺寸" className="!mb-0">
            <Select
              value={size}
          ...
          <Form.Item label="输出网格图" className="!mb-0">
            <Select
              value={String(candidateCount)}
              onChange={(v) => setCandidateCount(Number(v))}
              className="w-full"
              options={CANDIDATE_COUNTS.map((n) => ({
                value: String(n),
                label: `${n} 宫格（${
                  n === 1 ? "单图" : n === 2 ? "1×2" : n === 4 ? "2×2" : "3×3"
                }）`,
              }))}
            />
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Form.Item label="推荐生图模型（image-gen 工作台）" className="!mb-0">
            <Select
              value={model}
              onChange={(v) => setModel(v)}
              className="w-full"
              options={KNOWN_IMAGE_MODELS.map((m) => ({
                value: m.id,
                label: m.name,
              }))}
            />
          ...
          {/* Phase A 起新增：模板价格 */}
          <Form.Item label="价格（元）" className="!mb-0">
            <Input
              type="number"
              min={0}
              max={9999}
