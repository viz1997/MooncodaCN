...
import { seedanceReferenceLabel } from "@/features/canvas/lib/seedance-video";
...
export function buildNodeMentionReferences(
  ...
  return labelResourceNodes(
    getMentionResourceNodes(node.id, nodes, connections),
    true
  );
}
...
export async function resolveCanvasReferenceImages(
  ...
  return Promise.all(
    references
      ...
      .map(async (reference) => {
        ...
        if (!node)
          throw new Error(
            i18n.t("agent.composer.mentions.resourceMissing", {
        ...
        if (!dataUrl.startsWith("data:image/"))
          throw new Error(
            i18n.t("agent.composer.mentions.imageReadFailed", {
              title: reference.title,
            })
          );
        const meta =
...
export function getMentionResourceNodes(
  ...
  const configInputs = getConnectedConfigResourceNodes(
    nodeId,
    nodes,
    connections
  );
...
function getContextResourceNodes(
  ...
  return connections
    ...
    .map((connection) =>
      nodes.find((node) => node.id === connection.fromNodeId)
    )
    .filter((node): node is CanvasNodeData =>
      Boolean(node && isResourceNode(node))
    );
}
...
function getConnectedConfigResourceNodes(
  ...
  const configConnection = connections.find(
    (connection) =>
      ...
      nodes.find((node) => node.id === connection.toNodeId)?.type ===
        CanvasNodeType.Config
  );
  if (!configConnection) return [];
  return getContextResourceNodes(
    configConnection.toNodeId,
    nodes,
    connections
