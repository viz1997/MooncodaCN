...
import { createZip } from "@/features/canvas/lib/zip";
import { getMediaBlob } from "@/features/canvas/services/file-storage";
import { getImageBlob } from "@/features/canvas/services/image-storage";
import type { CanvasProject } from "@/features/canvas/stores/canvas/use-canvas-store";
import {
  type CanvasNodeData,
  CanvasNodeType,
...
import type {
  CanvasExportAsset,
  CanvasExportFile,
} from "@/features/canvas/types/canvas-export";
...
export async function exportCanvasProjects(
  ...
  const zip = await createZip([
    { name: "projects.json", data: JSON.stringify(data, null, 2) },
    ...zipFiles,
  ]);
  saveAs(zip, `${safeFileName(fileName)}.zip`);
}

export async function exportCanvasNodes(
  nodes: CanvasNodeData[],
  fileName = i18n.t("canvas.export.defaultNodesName")
) {
  const zipFiles: { name: string; data: BlobPart }[] = [];
      ...
      if (storageKey) {
        ...
        if (blob)
          return void zipFiles.push({
            name: uniqueName(title, fileExtension(blob.type, storageKey)),
            data: blob,
          });
      }
