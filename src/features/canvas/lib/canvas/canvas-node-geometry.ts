...
export function findGroupDropTarget(
  ...
  if (
    nodes.some(
      (node) => movedIds.has(node.id) && node.type === CanvasNodeType.Group
    )
  )
    return null;
  ...
  return (
    [...nodes].reverse().find((group) => {
      if (group.type !== CanvasNodeType.Group || movedIds.has(group.id))
        return false;
...
export function snapNodesIntoGroup(
  movedIds: Set<string>,
  nodes: CanvasNodeData[],
...
export function findContainingGroupId(
  node: CanvasNodeData,
  nodes: CanvasNodeData[]
) {
  const centerX = node.position.x + node.width / 2;
...
export function normalizeConnection(
  ...
  if (
    first.type === CanvasNodeType.Group ||
    second.type === CanvasNodeType.Group
  )
    return null;
  if (
