...
import { CanvasNodeType } from "@/features/canvas/types/canvas";
import type { CanvasNodeDefinition } from "@/features/canvas/types/canvas-plugin";
...
export const useNodeRegistryVersion = create<{ version: number }>(() => ({
  version: 0,
}));
function bump() {
...
const FALLBACK_SPEC = {
  width: 340,
  height: 240,
  title: i18n.t("canvas.node.node"),
  metadata: {} as CanvasNodeDefinition["defaultMetadata"],
};
