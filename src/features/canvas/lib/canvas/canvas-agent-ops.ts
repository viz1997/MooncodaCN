...
export function applyCanvasAgentOps(
  ...
  (Array.isArray(ops) ? ops : []).forEach((op, index) => {
    ...
    if (op.type === "add_node") {
      const nodeType =
        op.nodeType && isRegisteredNodeType(op.nodeType)
          ? op.nodeType
          : CanvasNodeType.Text;
      ...
      const ids = new Set(
        op.ids ||
          (op.id
            ...
            : op.nodeType
              ? nodes
                  .filter((node) => node.type === op.nodeType)
                  .map((node) => node.id)
              : [])
      );
    ...
    if (op.type === "delete_connections") {
      ...
      connections = op.all
        ? []
        : connections.filter((conn) => !ids.has(conn.id));
    }
    if (op.type === "connect_nodes") {
      if (!op.fromNodeId || !op.toNodeId) return;
      const exists = connections.some(
        (conn) =>
          conn.fromNodeId === op.fromNodeId && conn.toNodeId === op.toNodeId
      );
      const hasNodes =
    ...
    if (op.type === "select_nodes")
      selectedNodeIds = (op.ids || []).filter((id) =>
