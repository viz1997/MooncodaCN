...
import type { CanvasImageAngleParams } from "@/features/canvas/components/canvas/canvas-node-angle-dialog";
import type { NodeGenerationInput } from "@/features/canvas/components/canvas/canvas-node-generation";
import type { CanvasNodeGenerationMode } from "@/features/canvas/components/canvas/canvas-node-prompt-panel";
import i18n from "@/features/canvas/i18n";
import {
  imageMetadata,
  referenceUrl,
} from "@/features/canvas/lib/canvas/canvas-node-factory";
import { resolveMediaUrl } from "@/features/canvas/services/file-storage";
import {
  resolveImageUrl,
  uploadImage,
  ...
  return Promise.all(
    nodes.map(async (node) => {
      ...
      const images = await Promise.all(
        (node.metadata.images || []).map(async (image) =>
          image.content
            ? {
                ...image,
                content: await resolveImageUrl(image.storageKey, image.content),
              }
            : image
        )
      );
      ...
      return {
        ...
        metadata: {
          ...node.metadata,
          ...imageMetadata(await uploadImage(content)),
        },
      };
    })
  );
...
export function buildAngleLabel(params: CanvasImageAngleParams) {
  const horizontal =
    params.horizontalAngle === 0
      ...
      : params.horizontalAngle > 0
        ? i18n.t("canvas.generation.rotateRight", {
            angle: params.horizontalAngle,
          })
        : i18n.t("canvas.generation.rotateLeft", {
            angle: Math.abs(params.horizontalAngle),
          });
  const pitch =
    params.pitchAngle === 0
      ...
      : params.pitchAngle > 0
        ...
        : i18n.t("canvas.generation.lowAngle", {
            angle: Math.abs(params.pitchAngle),
          });
  return i18n.t("canvas.generation.angleLabel", {
    ...
    lens: i18n.t(
      params.wideAngle ? "canvas.editors.wide" : "canvas.editors.standard"
    ),
  });
}

export function buildAnglePrompt(params: CanvasImageAngleParams) {
  return i18n.t("canvas.generation.anglePrompt", {
    angle: buildAngleLabel(params),
  });
}
