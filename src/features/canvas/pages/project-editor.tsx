...
import { useNavigate, useParams, useSearchParams } from "next/navigation";
import type {
...
import { CanvasConfigComposer } from "@/features/canvas/components/canvas/canvas-config-composer";
import { CanvasConfigNodePanel } from "@/features/canvas/components/canvas/canvas-config-node-panel";
import {
  ActiveConnectionPath,
  ConnectionPath,
} from "@/features/canvas/components/canvas/canvas-connections";
import { CanvasNodeContextMenu } from "@/features/canvas/components/canvas/canvas-context-menu";
import {
...
} from "@/features/canvas/components/canvas/canvas-node-angle-dialog";
import {
  type CanvasImageCropRect,
  CanvasNodeCropDialog,
} from "@/features/canvas/components/canvas/canvas-node-crop-dialog";
import {
...
} from "@/features/canvas/components/canvas/canvas-node-generation";
import {
  CanvasNodeHoverToolbar,
  CanvasNodeInfoModal,
} from "@/features/canvas/components/canvas/canvas-node-hover-toolbar";
import {
  type CanvasImageMaskEditPayload,
  CanvasNodeMaskEditDialog,
} from "@/features/canvas/components/canvas/canvas-node-mask-edit-dialog";
import {
  type CanvasNodeGenerationMode,
  CanvasNodePromptPanel,
} from "@/features/canvas/components/canvas/canvas-node-prompt-panel";
import {
  type CanvasImageSplitParams,
  CanvasNodeSplitDialog,
} from "@/features/canvas/components/canvas/canvas-node-split-dialog";
import {
  type CanvasImageUpscaleParams,
  CanvasNodeUpscaleDialog,
} from "@/features/canvas/components/canvas/canvas-node-upscale-dialog";
import { CanvasPluginManagerModal } from "@/features/canvas/components/canvas/canvas-plugin-manager-modal";
import { CanvasRefreshShell } from "@/features/canvas/components/canvas/canvas-refresh-shell";
import { CanvasSidePanel } from "@/features/canvas/components/canvas/canvas-side-panel";
import { CanvasToolbar } from "@/features/canvas/components/canvas/canvas-toolbar";
import { CanvasTopBar } from "@/features/canvas/components/canvas/canvas-top-bar";
...
import {
  getNodeSpec,
  NODE_DEFAULT_SIZE,
} from "@/features/canvas/constant/canvas";
import { exportCanvasProjects } from "@/features/canvas/lib/canvas/canvas-export";
import {
  audioExtension,
  buildAngleLabel,
  buildAnglePrompt,
...
} from "@/features/canvas/lib/canvas/canvas-generation-helpers";
import {
  cropDataUrl,
  splitDataUrl,
  upscaleDataUrl,
} from "@/features/canvas/lib/canvas/canvas-image-data";
import {
  ...
  imageMetadata,
  videoMetadata,
} from "@/features/canvas/lib/canvas/canvas-node-factory";
import {
  ...
  normalizeConnection,
  snapNodesIntoGroup,
} from "@/features/canvas/lib/canvas/canvas-node-geometry";
import {
  fitNodeSize,
  nodeSizeFromRatio,
} from "@/features/canvas/lib/canvas/canvas-node-size";
import {
  buildNodeMentionReferences,
  type CanvasResourceReference,
} from "@/features/canvas/lib/canvas/canvas-resource-references";
import {
  getNodeDefinition,
...
import { useAgentBridge } from "@/features/canvas/pages/hooks/use-agent-bridge";
import { usePluginHost } from "@/features/canvas/pages/hooks/use-plugin-host";
import {
  requestAudioGeneration,
  storeGeneratedAudio,
} from "@/features/canvas/services/api/audio";
import {
  requestEdit,
  requestGeneration,
...
import {
  ...
  storeGeneratedVideo,
} from "@/features/canvas/services/api/video";
import { uploadMediaFile } from "@/features/canvas/services/file-storage";
import { uploadImage } from "@/features/canvas/services/image-storage";
import { useCanvasStore } from "@/features/canvas/stores/canvas/use-canvas-store";
import { useAgentStore } from "@/features/canvas/stores/use-agent-store";
...
import type { ReferenceAudio } from "@/features/canvas/types/media";
// useNavigate 是 React Router API（infinite-canvas 原项目用 React Router 7），
// Next.js 的 next/navigation 没有这个 hook；改用 next-intl 包装的 useRouter（自动加 locale 前缀）
...
function InfiniteCanvasPage() {
  ...
  // 原代码用 React Router 的 useNavigate，但 next/navigation 没有这个 hook；
  // 用 next-intl 包装的 useRouter（自动加 locale 前缀）替代
  const router = useI18nRouter();
  // 不要用 `router.push.bind(router)` —— Function.prototype.bind 每次调用都返回新函数引用，
  // 放进 useEffect deps 会让 effect 在每次渲染都触发 setProjectLoaded(false) → re-render → 死循环
  // 直接用 router.push / router.replace（next-intl 返回的 router 对象稳定，方法也是稳定引用）
  const navigate = router.push;
  const navigateReplace = router.replace;
  ...
  const [nodeCreatePosition, setNodeCreatePosition] = useState<Position | null>(
    null
  );
  ...
  const [expandedImageNodeId, setExpandedImageNodeId] = useState<string | null>(
    null
  );
  ...
  const generationRequestsRef = useRef(
    new Map<string, CanvasGenerationRequest>()
  );
  ...
  useEffect(() => {
    ...
    if (!project) {
      navigateReplace("/dashboard/canvas");
      return;
    }

    const restore = async () => {
      const restoredNodes = await hydrateCanvasImages(
        resetInterruptedGeneration(project.nodes)
      );
      const restoredSessions = await hydrateAssistantImages(
  ...
  useEffect(() => {
    ...
    // useSearchParams() 在 Suspense 切换 / 部分预渲染时可能短暂返回 null
    // → 解构出 undefined。所有访问都走可选链，searchParams 未就绪时直接 return
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      // 取整避免浮点尾数差异导致 ResizeObserver 反馈环
      const width = Math.round(rect.width);
      const height = Math.round(rect.height);
      // setState 携带新对象引用会让 React 误以为"变了"，触发 re-render →
      // 父容器布局变化 → ResizeObserver 又 fire → setSize 再次被调用 →
      // Maximum update depth exceeded。
      // 用函数式 setState + 比较 prev，只有真变了才返新对象，否则返 prev
      // → React 自动 bail-out，不重渲染
      setSize((prev) =>
  ...
  const keepNodeToolbar = useCallback(
    (nodeId: string) => {
      if (
        ...
        !selectedNodeIdsRef.current.has(nodeId)
      )
        return;
  ...
  const connectNodes = useCallback(
    (current: ConnectionHandle, targetNodeId: string) => {
      ...
      if (!exists) {
        setConnections((prev) => [
          ...prev,
          { id: `conn-${Date.now()}`, fromNodeId, toNodeId },
        ]);
      }
      setContextMenu(null);
    },
  ...
  const { pluginHost, renderPluginPanel, buildNodeToolbarItems } =
    usePluginHost({
      effectiveConfig,
      isAiConfigReady,
      theme,
    ...
    (ids: Set<string>) => {
      ...
      setNodes((prev) => {
        ...
        return next.map((node) => {
          const groupId = node.metadata?.groupId;
          if (groupId && allIds.has(groupId))
            return {
              ...node,
              metadata: { ...node.metadata, groupId: undefined },
            };
          return node;
        });
      });
  ...
  const pasteCopiedNodes = useCallback(() => {
    ...
    const pastedNodes = nextNodes.map((node) => {
      ...
      return {
        ...node,
        metadata: { ...node.metadata, groupId: idMap.get(groupId) },
      };
    });
    ...
    setContextMenu(null);
    setDialogNodeId(
      pastedNodes[0]?.type === CanvasNodeType.Group
        ? null
        : pastedNodes[0]?.id || null
    );
    return true;
  }, [getCanvasCenter]);
  ...
  const focusNode = useCallback(
    (nodeId: string) => {
      ...
      const k = Math.min(
        Math.max(
          Math.min(
            (size.width * 0.6) / node.width,
            (size.height * 0.6) / node.height
          ),
          0.05
        ),
        1
  ...
  useEffect(
    () => () =>
      void (focusAnimRef.current && cancelAnimationFrame(focusAnimRef.current)),
    []
  );
  ...
  const createAndOpenProject = useCallback(() => {
    const id = createProject(
      t("canvas.defaultTitle", {
        count: useCanvasStore.getState().projects.length + 1,
      })
    );
  ...
  const selectNodeByEvent = useCallback(
    (
      ...
      const soloId =
        nextSelected.size === 1 && nextSelected.has(nodeId) ? nodeId : null;
  ...
  const handleNodeMouseDown = useCallback(
    (event: ReactMouseEvent, nodeId: string) => {
      ...
      const nextSelected =
        ...
        selectNodeByEvent(event, nodeId).nextSelected;
  ...
  const finishNodeDrag = useCallback((clientX?: number, clientY?: number) => {
    ...
    if (dragRef.current.hasMoved && clientX != null && clientY != null) {
      ...
      setNodes((prev) => {
        ...
        if (targetGroup)
          return snapNodesIntoGroup(movedIds, moved, targetGroup);
        return moved.map((node) => {
          if (!movedIds.has(node.id) || node.type === CanvasNodeType.Group)
            return node;
    ...
    if (wasClick && clickedNodeId) {
      ...
      const clickedDefinition = clickedNode
        ? getNodeDefinition(clickedNode.type)
        : undefined;
    ...
    (event: MouseEvent) => {
      ...
      if (dragRef.current.isDraggingNode) {
        ...
        setDropTargetGroupId(
          findGroupDropTarget(movedIds, previewNodes)?.id || null
        );
  ...
  const handleNodeContentChange = useCallback(
    (nodeId: string, content: string) => {
      setNodes((prev) =>
        prev.map((node) =>
          node.id === nodeId
            ? { ...node, metadata: { ...node.metadata, content } }
            : node
        )
      );
    },
  ...
  const setBatchPrimary = useCallback((nodeId: string, imageId: string) => {
    setNodes((prev) =>
      prev.map((node) => {
        if (node.id !== nodeId) return node;
        const image = node.metadata?.images?.find(
  ...
  const duplicateBatchImage = useCallback(
    (node: CanvasNodeData, imageId: string) => {
      ...
      const copy: CanvasNodeData = {
        ...
        title: node.title,
        position: {
          x: node.position.x + node.width * 2 + 96,
          y: node.position.y + node.height / 2 - size.height / 2,
        },
        ...size,
        metadata: {
  ...
  const downloadBatchImage = useCallback(
    (node: CanvasNodeData, imageId: string) => {
      ...
      saveAs(
        image.content,
        `canvas-image-${node.id}-${image.id}.${imageExtension(image.content)}`
      );
    },
    []
  );
  ...
  const saveNodeAsset = useCallback(
    async (node: CanvasNodeData) => {
      if (node.type === CanvasNodeType.Text) {
        ...
        if (!content)
          return message.error(t("canvas.projectPage.noTextToSave"));
        addAsset({
          kind: "text",
          title:
            node.metadata?.prompt?.slice(0, 24) ||
            t("canvas.projectPage.canvasText"),
          coverUrl: "",
          tags: [],
          source: "Canvas",
          data: { content },
      ...
      if (node.type === CanvasNodeType.Video) {
        if (!node.metadata?.content)
          return message.error(t("canvas.projectPage.noVideoToSave"));
        addAsset({
          kind: "video",
          title:
            node.metadata?.prompt?.slice(0, 24) ||
            t("canvas.projectPage.canvasVideo"),
          coverUrl: "",
          tags: [],
          source: "Canvas",
          data: {
      ...
      if (!node.metadata?.content)
        return message.error(t("canvas.projectPage.noImageToSave"));
      ...
      const configNode = {
        ...createCanvasNode(
          ...
          },
          {
            ...
            count: 1,
            composerContent: t("canvas.reverseComposer", {
              imageId: node.id,
              textId: textNode.id,
            }),
          }
  ...
  const splitImageNode = useCallback(
    async (node: CanvasNodeData, params: CanvasImageSplitParams) => {
      ...
      setDialogNodeId(null);
      message.success(
        t("canvas.projectPage.splitSuccess", { count: childNodes.length })
      );
    },
    [message, t]
  );
      ...
      try {
        ...
        const errorDetails =
          error instanceof Error
            ? error.message
            : t("canvas.projectPage.maskFailed");
    ...
    async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
      ...
      try {
        ...
        const errorDetails =
          error instanceof Error
            ? error.message
            : t("canvas.projectPage.generationFailed");
        setNodes((prev) =>
          prev.map((item) =>
            item.id === childId
              ? {
    ...
    async (event: ReactChangeEvent<HTMLInputElement>) => {
      ...
      if (target?.nodeId) {
        ...
        for (let i = 0; i < rest.length; i++) {
          const offsetPos = {
            x: basePosition.x + (i + 1) * STAGGER,
            y: basePosition.y + (i + 1) * STAGGER,
          };
          const f = rest[i];
          if (isAudioFile(f)) {
        ...
        for (let i = 0; i < files.length; i++) {
          const offsetPos = {
            x: basePosition.x + i * STAGGER,
            y: basePosition.y + i * STAGGER,
          };
          const f = files[i];
          if (isAudioFile(f)) {
            void createAudioFileNode(f, offsetPos);
  ...
  const handleGenerateNode = useCallback(
    async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
      ...
      const builtinPanel = sourceNode
        ? getNodeDefinition(sourceNode.type)?.useBuiltinPanel
        : undefined;
      if (
        sourceNode &&
        builtinPanel?.writeBackToSelf &&
        builtinPanel.mode === "image"
      ) {
        ...
        try {
          ...
          const upstreamNodes = connectionsRef.current
            ...
            .map((conn) =>
              nodesRef.current.find((node) => node.id === conn.fromNodeId)
            )
            .filter((node): node is CanvasNodeData => Boolean(node));
          const refs = upstreamNodes.flatMap((up) =>
            typeof up.metadata?.content === "string" &&
            up.metadata.content &&
            up.type !== sourceNode.type
              ? [
                  {
                    id: up.id,
      ...
      const generationContext = await hydrateNodeGenerationContext(
        buildNodeGenerationContext(
          ...
          editingTextNode
            ? t("canvas.projectPage.editTextPrompt", {
                source: sourceTextContent,
                prompt,
              })
            : prompt
        )
      ...
      try {
        if (mode === "image") {
          ...
          const rootNode: CanvasNodeData = {
            ...
            position: {
              x: isEmptyImageNode
                ? parentPosition.x
                : parentPosition.x + parentConfig.width + 96,
              y:
                parentPosition.y +
                parentConfig.height / 2 -
                imageConfig.height / 2,
            },
            width: isEmptyImageNode
              ? sourceNode?.width || imageConfig.width
              : imageConfig.width,
          ...
          const controller =
            rootId === nodeId
              ? runController
              : startGenerationRequest(rootId, nodeId, nodeId, runController);
          let hasSuccess = false;
          let hasFailure = false;
          let firstError = "";
          // 一次接口请求返多张（与 workbench 同语义，n=count 一次返 count 张）。
          // 之前这里是 Promise.all × count 个并行 request，每个都把 count 拍成 "1"，
          // 触发 4 个 POST /api/canvas/generate；现在合并成 1 个 POST，
          // 由后端 batchSize=n 内部并行 submitLingtingTask 处理（gpt-image-2 一次只返 1 张）。
          let generatedItems: Array<{ id: string; dataUrl: string }> = [];
          let requestError: Error | null = null;
          try {
            generatedItems = referenceImages.length
              ? await requestEdit(
                  generationConfig,
                  effectivePrompt,
                  referenceImages,
            ...
            requestError =
              err instanceof Error
                ? err
                : new Error(t("canvas.projectPage.generationFailed"));
            if (!firstError) firstError = requestError.message;
          }

          await Promise.all(
            imageIds.map(async (imageId, idx) => {
              try {
                if (requestError) throw requestError;
                const imageItem = generatedItems[idx];
                if (!imageItem) {
                  // 后端返回图数不足（n=count 但只返了 idx 张之前的）—— 该槽失败
                  throw new Error(t("canvas.projectPage.generationFailed"));
                }
                const uploaded = await uploadImage(imageItem.dataUrl);
                  ...
                  prev.map((node) => {
                    ...
                    const images =
                      node.metadata?.images?.map((image) =>
                        image.id === imageId ? item : image
                      ) || [];
                    if (node.metadata?.primaryImageId)
                      return {
                        ...node,
                        metadata: { ...node.metadata, images },
                      };
                    const center = {
                ...
                : node.id === rootId
                  ? {
                      ...
                      metadata: {
                        ...
                        errorDetails: hasSuccess
                          ? undefined
                          : t("canvas.projectPage.allFailed"),
                      },
                    }
                  : node
            )
        ...
        if (mode === "video") {
          ...
          const controller = startGenerationRequest(
            videoId,
            nodeId,
            nodeId,
            runController
  ...
  const handleRetryNode = useCallback(
    async (node: CanvasNodeData, imageId?: string) => {
      ...
      const savedImageMetadata =
        node.type === CanvasNodeType.Image ? node.metadata : undefined;
      ...
      const generationConfig =
        hasSavedImageMetadata && savedImageMetadata
          ? {
              ...
              background:
                savedImageMetadata.background ?? effectiveConfig.background,
              count: "1",
            }
          : {
              ...buildGenerationConfig(
          ...
          prev.map((item) =>
            item.id === node.id
              ? {
                  ...
                  metadata: {
                    ...item.metadata,
                    status: item.metadata?.content
                      ? NODE_STATUS_SUCCESS
                      : NODE_STATUS_ERROR,
                    errorDetails: item.metadata?.content
                      ? undefined
                    ...
                    images: item.metadata?.images?.map((image) =>
                      image.id === imageId
                        ? {
                            ...
                            status: NODE_STATUS_ERROR,
                            errorDetails: t(
                              "canvas.projectPage.referenceMissing"
                            ),
                          }
                        : image
                    ),
            ...
            return {
              ...
              metadata: {
                ...
                ...(makePrimary
                  ...
                  : { status: NODE_STATUS_SUCCESS }),
                images: item.metadata?.images?.map((current) =>
                  current.id === retryImage.id ? retryImage : current
                ),
                primaryImageId: makePrimary
                  ? retryImage.id
                  : item.metadata?.primaryImageId,
                prompt,
                ...generationMetadata,
              },
            };
  ...
  const deleteBatchImage = useCallback((nodeId: string, imageId: string) => {
    const node = nodesRef.current.find((item) => item.id === nodeId);
    if ((node?.metadata?.images?.length || 0) <= 2)
      setExpandedImageNodeId(null);
    setNodes((prev) =>
      prev.map((item) => {
        if (item.id !== nodeId) return item;
        const images =
          item.metadata?.images?.filter((image) => image.id !== imageId) || [];
        return {
          ...item,
          metadata: {
            ...item.metadata,
            ...
            count: images.length,
            primaryImageId:
              item.metadata?.primaryImageId === imageId
                ? images[0]?.id
                : item.metadata?.primaryImageId,
          },
        };
  ...
  const retryBatchImage = useCallback(
    (node: CanvasNodeData, imageId: string) =>
      void handleRetryNode(node, imageId),
    [handleRetryNode]
  );
  ...
  const handleNodeViewImage = useCallback(
    (node: CanvasNodeData) => setPreviewNodeId(node.id),
    []
  );
  const handleNodeRetry = useCallback(
    (node: CanvasNodeData) => void handleRetryNode(node),
    [handleRetryNode]
  );
    ...
    (panelNode: CanvasNodeData) =>
      getNodeDefinition(panelNode.type)?.Panel ? (
        ...
        <CanvasNodePromptPanel
          ...
          mentionReferences={
            mentionReferencesByNodeId.get(panelNode.id) || EMPTY_REFERENCES
          }
          ...
          modeOverride={
            getNodeDefinition(panelNode.type)?.useBuiltinPanel?.mode
          }
    ...
    >
      ...
      <section className="relative min-w-0 flex-1 overflow-hidden">
        <CanvasTopBar
          ...
          // "Home" 按钮以前跳 /，登录态会被 middleware 重定向到 /dashboard。
          // 改成跳画布项目列表，跟 Projects 按钮同指向，用户体感是
          // "首页 = 画布列表"（对画布内用户而言，列表页就是他们的"首页"）
          onHome={() => navigate("/dashboard/canvas")}
          onProjects={() => navigate("/dashboard/canvas")}
          ...
          {visibleNodes.map((node) => (
            <CanvasNode
              ...
              showPanel={
                ...
                !selectionBox &&
                !getNodeDefinition(node.type)?.hidePanel
              }
        ...
        <CanvasNodeHoverToolbar
          ...
          viewport={viewport}
          extraTools={
            toolbarNode ? buildNodeToolbarItems(toolbarNode) : undefined
          }
        ...
        >
          <div className="py-8 text-center text-base font-medium">
            {t("canvas.projectPage.notImplemented")}
          </div>
        </Modal>
        ...
        >
          <p className="text-sm opacity-60">
            {t("canvas.projectPage.clearDescription")}
          </p>
        </Modal>
