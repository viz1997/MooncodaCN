...
export async function uploadMediaFile(
  ...
  // Phase 3：内置渠道已经返回 R2 永久 URL —— 不再走 localforage，直接用 URL
  if (typeof input === "string" && /^https?:\/\//i.test(input)) {
    ...
    try {
      meta =
        prefix === "video"
          ? await readVideoMeta(input)
          : prefix === "audio"
            ? await readAudioMeta(input)
            : {};
    } catch {
      // R2 偶尔 CORS 不通时 meta 拉不到 —— 忽略，前端仍能用 <video>/<audio> 渲染
    }
    return {
      url: input,
      storageKey: `r2:${input}`,
...
export function collectMediaStorageKeys(
  value: unknown,
  keys = new Set<string>()
) {
  ...
  Object.values(value).forEach((item) =>
    Array.isArray(item)
      ? item.forEach((child) => collectMediaStorageKeys(child, keys))
      : collectMediaStorageKeys(item, keys)
  );
  return keys;
}
