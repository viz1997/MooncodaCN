...
import { type RawPrompt, runPromptSource } from "./prompt-source-runtime";
...
const promptCacheStore = localforage.createInstance({
  name: "infinite-canvas",
  storeName: "prompt_cache",
});
const loadingSources = new Map<string, Promise<PromptSourceRefreshResult>>();

function enabledSources() {
  return usePromptSourceStore
    .getState()
...
export async function fetchSourcePrompts(sourceId: string): Promise<Prompt[]> {
  const source = usePromptSourceStore
    .getState()
...
export async function fetchPromptSourceStatuses(): Promise<
  Record<string, PromptSourceStatus>
> {
