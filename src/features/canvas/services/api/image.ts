...
const GEMINI_IMAGE_SIZE_BY_QUALITY: Record<string, string> = {
  low: "1K",
  medium: "2K",
  high: "4K",
  standard: "1K",
...
function resolveSize(quality: string | undefined, ratio: string): string {
  ...
  if (basePixels) {
    ...
    shortSide =
      Math.round(longSide / longRatio / IMAGE_SIZE_STEP) * IMAGE_SIZE_STEP;
  } else {
    shortSide = DEFAULT_IMAGE_SHORT_SIDE;
    longSide =
      Math.round((shortSide * longRatio) / IMAGE_SIZE_STEP) * IMAGE_SIZE_STEP;
  }
...
function parseRatioValue(value: string) {
  ...
  if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0)
    throw new Error(apiText("positiveImageRatio"));
...
function parseImageRatio(value: string) {
  ...
  if (
    ...
    throw new Error(apiText("imageRatioLimit"));
  return ratio;
}
...
function resolveGeminiImageConfig(config: AiConfig) {
  ...
  const aspectRatio =
    value && value.toLowerCase() !== "auto"
      ? closestGeminiAspectRatio(ratio)
      : undefined;
  const imageSize = supportsGeminiImageSize(config.model)
    ? resolveGeminiImageSize(config.quality, dimensions)
    : undefined;
  const image = {
...
function parseImagePayload(payload: ImageApiResponse) {
  ...
  const imageList =
    payload.data ||
    ((payload as Record<string, unknown>).images as
  ...
  if (images.length === 0) {
    ...
    const rawKeys = Object.keys(payload).filter(
      (k) => k !== "code" && k !== "msg" && k !== "error"
    );
    throw new Error(
      rawKeys.length > 0
...
function withSystemMessage<T extends ResponseInputMessage>(
  config: AiConfig,
  messages: T[]
...
function toResponseInput(
  ...
  return messages.flatMap((message): ResponseInputItem[] => {
    ...
    return [
      { role: message.role, content: toResponseContent(message.content || "") },
    ];
  });
}
...
function toResponseContent(
  ...
  return content.map((item) =>
    item.type === "text"
      ...
      : { type: "input_image" as const, image_url: item.image_url.url }
  );
}
...
function parseToolResponse(payload: ResponseApiPayload): ToolResponseResult {
  ...
  const toolCalls = output
    .filter(
      (
        item
      ): item is Extract<ResponseApiOutputItem, { type?: "function_call" }> =>
...
function validateGeminiPayload(payload: GeminiPayload) {
  ...
  if (payload.promptFeedback?.blockReason)
    throw new Error(
      apiText("geminiRejected", { reason: payload.promptFeedback.blockReason })
    );
}
...
async function readFetchError(response: Response, fallback: string) {
  ...
  try {
    return (
      responseErrorMessage(JSON.parse(text)) ||
      readStatusError(response.status, fallback)
    );
  } catch {
...
async function requestStreamingResponse(
  ...
  if (!response.ok)
    throw new Error(await readFetchError(response, apiText("requestFailed")));
  if (!response.body) {
...
function toGeminiBody(
  ...
  const systemText = [
    ...
    ...messages.flatMap((message) =>
      !("type" in message) && message.role === "system"
        ? [geminiTextContent(message.content)]
        : []
    ),
  ]
    .filter(Boolean)
  ...
  const contents = toGeminiContents(
    messages.filter((message) =>
      "type" in message ? true : message.role !== "system"
    )
  );
  return {
    contents,
          ...
          parts: [
            {
              functionCall: {
                id: message.call_id,
                name: message.name,
                args: jsonObject(message.arguments),
              },
          ...
          parts: [
            {
              functionResponse: {
                ...
                name,
                response: { result: jsonValue(message.content) },
              },
            },
          ],
        },
...
function toGeminiParts(content: ResponseMessageContent): GeminiPart[] {
  ...
  return content.map((item) =>
    item.type === "text"
      ? { text: item.text }
      : toGeminiImagePart(item.image_url.url)
  );
}
...
async function requestGeminiStreamingResponse(
  config: AiConfig,
  body: Record<string, unknown>,
  onDelta?: (text: string) => void,
  options?: RequestOptions
...
function parseGeminiToolResponse(payload: GeminiPayload): ToolResponseResult {
  ...
  const toolCalls = parts
    ...
    .filter((call): call is NonNullable<GeminiPart["functionCall"]> =>
      Boolean(call?.name)
    )
    .map((call) => {
...
async function requestGeminiImages(
  ...
  const requests = Array.from({ length: count }, () =>
    requestGeminiImagesOnce(config, prompt, references, options)
  );
      ...
      .map((part) => {
        const inlineData =
          ...
          (part.inline_data
            ? {
                mimeType:
                  part.inline_data.mimeType || part.inline_data.mime_type,
...
export async function requestGeneration(
  ...
  // Phase 3：内置渠道走 /api/canvas/generate 代理（系统 Key + 积分扣减 + R2 永久化）
  if (config.channelMode === "remote") {
    return requestRemoteImage({
      capability: "image",
      mode: "generation",
      config,
      prompt,
  ...
  const n = Math.max(
    1,
    Math.min(15, Math.floor(Math.abs(Number(config.count)) || 1))
  );
  ...
  if (script) {
    ...
    try {
      ...
      return normalizePluginImages(result).map((dataUrl) => ({
        id: nanoid(),
        dataUrl,
      }));
    } catch (error) {
...
export async function requestEdit(
  ...
  if (script) {
    ...
    const refs = await Promise.all(
      references.map((image) => imageToDataUrl(image))
    );
    try {
      const result = await runModelPlugin({
        capability: "image",
...
export async function requestImageQuestion(
  ...
  try {
    if (requestConfig.apiFormat === "gemini") {
      const answer =
        (
          await requestGeminiStreamingResponse(
            requestConfig,
            toGeminiBody(requestConfig, messages),
            onDelta,
            options
          )
      ...
      (
        await requestStreamingResponse(
          requestConfig,
          {
            ...
            ...(requestConfig.reasoningEffort === "auto"
              ? {}
              : { reasoning: { effort: requestConfig.reasoningEffort } }),
          },
          onDelta,
          options
        )
...
// ───────────────────────────────────────────────────────────────────────────
// Phase 4：内置渠道（channelMode === "remote"）走两段式
//   POST /api/canvas/generate → { jobId }
//   GET  /api/canvas/poll/{jobId} → { status, items?, message? }
// 后端在 create 时写 canvasRemoteJob（status=pending） + inngest.send，
// Inngest 函数 `canvasRemoteGenerateJob` 后台跑生成 + R2 + 失败 refund。
// 前端无需处理积分。
// ───────────────────────────────────────────────────────────────────────────

const IMAGE_POLL_DELAY_MS = 3000;
const IMAGE_POLL_MAX_ATTEMPTS = 120; // ≈ 6min（覆盖 gpt-image-2 最坏情况）

async function requestRemoteImage(params: {
  capability: "image";
...
async function createRemoteImageJob(params: {
  ...
  if (!response.ok || !body.success || !body.jobId) {
    throw new Error(body.error || `内置渠道请求失败：HTTP ${response.status}`);
  }

  return body.jobId;
}

async function pollRemoteImageJob(
  jobId: string,
  signal?: AbortSignal
): Promise<{
  status: "completed";
  ...
  for (let attempt = 0; attempt < IMAGE_POLL_MAX_ATTEMPTS; attempt += 1) {
    ...
    await delay(IMAGE_POLL_DELAY_MS, signal);

    const pollRes = await fetch(`/api/canvas/poll/${jobId}`, {
      credentials: "include",
      signal,
    });
    ...
    if (!pollRes.ok || !pollBody.success) {
      throw new Error(
        pollBody.error || `内置渠道轮询失败：HTTP ${pollRes.status}`
      );
    }
...
const defaultGeminiConfig: Pick<
  AiConfig,
