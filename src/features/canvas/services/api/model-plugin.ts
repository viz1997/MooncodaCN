  ...
  const run = async (
    ...
    const response = await axios.request({
      ...
      headers: pluginHeaders(
        { Authorization: `Bearer ${config.apiKey}`, ...opts?.headers },
        method === "post" && !isForm && body !== undefined
      ),
...
function createPoll(signal?: AbortSignal) {
  return async function poll<T, R>(
    request: () => Promise<T>,
    ...
    for (;;) {
      ...
      if (performance.now() >= deadline)
        throw new Error(i18n.t("modelPlugin.pollTimeout"));
 ...
 */
export async function runModelPlugin<T = unknown>(
  args: RunPluginArgs
): Promise<T> {
  const { config } = args;
  ...
  return [
    {
      name: "prompt",
      type: "string",
      desc: i18n.t("modelPlugin.variables.prompt"),
      capabilities: ["image", "video", "audio"],
    },
    {
      name: "images",
      type: "string[]",
      desc: i18n.t("modelPlugin.variables.images"),
      capabilities: ["image", "video"],
    },
    {
      name: "messages",
      type: "{ role, content }[]",
      desc: i18n.t("modelPlugin.variables.messages"),
      capabilities: ["text"],
    },
    {
      name: "params",
      type: "object",
      desc: i18n.t("modelPlugin.variables.params"),
    },
    {
      name: "model",
      type: "string",
      desc: i18n.t("modelPlugin.variables.model"),
    },
    {
      name: "baseUrl",
      type: "string",
      desc: i18n.t("modelPlugin.variables.baseUrl"),
    },
    {
      name: "apiKey",
      type: "string",
      desc: i18n.t("modelPlugin.variables.apiKey"),
    },
    {
      name: "systemPrompt",
      type: "string",
      desc: i18n.t("modelPlugin.variables.systemPrompt"),
    },
    {
      name: "reasoningEffort",
      type: '"auto" | "low" | "medium" | "high" | "xhigh"',
      desc: i18n.t("modelPlugin.variables.reasoningEffort"),
      capabilities: ["text"],
    },
    {
      name: "http",
      type: "object",
      desc: i18n.t("modelPlugin.variables.http"),
    },
    {
      name: "request",
      type: "function",
      desc: i18n.t("modelPlugin.variables.request"),
    },
    {
      name: "poll",
      type: "function",
      desc: i18n.t("modelPlugin.variables.poll"),
    },
    {
      name: "sleep",
      type: "function",
      desc: i18n.t("modelPlugin.variables.sleep"),
    },
    {
      name: "signal",
      type: "AbortSignal",
      desc: i18n.t("modelPlugin.variables.signal"),
    },
    {
      name: "onDelta",
      type: "function",
      desc: i18n.t("modelPlugin.variables.onDelta"),
      capabilities: ["text"],
    },
  ];
}
...
export function getPluginTemplates(): Record<
  ModelCapability,
  PluginTemplate[]
> {
  return {
