...
} from "@/features/canvas/lib/seedance-video";
import {
  getMediaBlob,
  type UploadedFile,
  uploadMediaFile,
...
import type {
  ReferenceAudio,
  ReferenceVideo,
} from "@/features/canvas/types/media";
...
export async function requestVideoGeneration(
  ...
  // Phase 3：内置渠道走 /api/canvas/generate（创建） + /api/canvas/poll（轮询）
  if (config.channelMode === "remote") {
    return requestRemoteVideo(config, prompt, options?.signal);
  }
  ...
  for (let attempt = 0; attempt < 120; attempt += 1) {
    ...
    if (attempt === 119)
      throw new Error(
        apiText("videoTimeout", {
          provider: task.provider === "seedance" ? "Seedance " : "",
        })
      );
...
export async function pollVideoGenerationTask(
  ...
  if (task.provider === "plugin") {
    ...
    return result
      ...
      : { status: "failed", error: apiText("pluginVideoExpired") };
  }
...
function videoPluginResult(result: unknown): VideoGenerationResult {
  ...
  if (result && typeof result === "object") {
    ...
    const url = [record.url, record.video_url, record.result_url].find(
...
export async function storeGeneratedVideo(
  ...
  // Phase 3：内置渠道返的 result.url 是 R2 永久 URL，无需再 uploadMediaFile
  if (result.url && result.url.startsWith("http")) {
    return {
      url: result.url,
      storageKey: `r2:${result.url}`,
...
// ───────────────────────────────────────────────────────────────────────────
// Phase 3：内置渠道（channelMode === "remote"）走两段式
//   POST /api/canvas/generate → { jobId }
//   GET  /api/canvas/poll/{jobId} → { status: "pending" | "completed" | "failed", items?, message? }
// 后端在 create 时预扣积分，failed 时自动 refund；前端无需处理积分
// ───────────────────────────────────────────────────────────────────────────

const VIDEO_POLL_DELAY_MS = 4000;
const VIDEO_POLL_MAX_ATTEMPTS = 90; // ≈ 6min
...
async function requestRemoteVideo(
  ...
  // 1. 创建任务
  const createRes = await fetch("/api/canvas/generate", {
    method: "POST",
  ...
  if (!createRes.ok || !createBody.success || !createBody.jobId) {
    throw new Error(
      createBody.error || `内置视频创建失败：HTTP ${createRes.status}`
    );
  }
  ...
  for (let attempt = 0; attempt < VIDEO_POLL_MAX_ATTEMPTS; attempt += 1) {
    ...
    await delay(VIDEO_POLL_DELAY_MS, signal);
    ...
    if (!pollRes.ok || !pollBody.success) {
      throw new Error(
        pollBody.error || `内置视频轮询失败：HTTP ${pollRes.status}`
      );
    }

    if (pollBody.status === "completed" && pollBody.items?.[0]) {
      return {
        url: pollBody.items[0].url,
        mimeType: pollBody.items[0].mimeType || "video/mp4",
      };
    }

    if (pollBody.status === "failed") {
      throw new Error(pollBody.message || "内置视频生成失败（积分已自动回退）");
    }
  }
  throw new Error(apiText("videoTimeout", { provider: "" }));
}
...
async function createOpenAIVideoTask(
  ...
  if (normalizeVideoSize(config.size))
    body.append("size", normalizeVideoSize(config.size)!);
...
async function pollOpenAIVideoTask(
  ...
  try {
    const video = unwrapVideoResponse(
      (
        await axios.get<ApiVideoResponse>(
          aiApiUrl(config, `/videos/${task.id}`),
    ...
    if (url)
      return {
        status: "completed",
        result: await videoResultFromUrl(url, options),
      };
    if (video.status === "completed") {
      const content = await axios.get<Blob>(
        aiApiUrl(config, `/videos/${task.id}/content`),
        {
          headers: aiHeaders(config),
          responseType: "blob",
          signal: options?.signal,
    ...
    if (video.status === "failed" || video.status === "cancelled")
      return {
        status: "failed",
        error:
          readApiErrorMessage(video.error?.message) ||
          apiText("videoGenerationFailed"),
      };
...
async function createSeedanceTask(
  ...
  try {
    const created = unwrapSeedanceTask(
      (
        await axios.post<ApiEnvelope<SeedanceTask>>(
          seedanceApiUrl(config),
          payload,
          {
...
async function pollSeedanceTask(
  ...
  try {
    const state = unwrapSeedanceTask(
      (
        await axios.get<ApiEnvelope<SeedanceTask>>(
          seedanceApiUrl(config, task.id),
          { headers: aiHeaders(config), signal: options?.signal }
        )
      ).data
    );
    ...
    if (state.status === "succeeded" || state.status === "completed")
      return { status: "failed", error: apiText("seedanceNoVideoUrl") };
    if (
      state.status === "failed" ||
      ...
      return {
        status: "failed",
        error:
          readApiErrorMessage(state.error?.message) ||
          apiText(
            state.status === "expired"
              ? "seedanceVideoTimeout"
              : "seedanceVideoFailed"
          ),
      };
    return { status: "pending" };
...
function assertSeedanceVideoReferences(videoReferences: ReferenceVideo[]) {
  ...
  for (const video of videoReferences) {
    ...
    if (video.durationMs < 2000 || video.durationMs > 15000)
      throw new Error(apiText("seedanceVideoDuration"));
...
function assertSeedanceAudioReferences(audioReferences: ReferenceAudio[]) {
  ...
  for (const audio of audioReferences) {
    ...
    if (audio.durationMs < 2000 || audio.durationMs > 15000)
      throw new Error(apiText("seedanceAudioDuration"));
...
async function buildSeedanceContent(
  ...
  for (const video of videoReferences.slice(
    0,
  ...
  for (const audio of audioReferences.slice(
    0,
...
async function resolveSeedanceAudioUrl(audio: ReferenceAudio) {
  ...
  if (!blob && audio.url?.startsWith("blob:"))
    blob = await (await fetch(audio.url)).blob();
...
function assertVideoConfig(config: AiConfig, model: string) {
  ...
  if (config.apiFormat === "gemini")
    throw new Error(apiText("geminiVideoUnsupported"));
}
...
function videoResultUrl(payload: VideoResponse | SeedanceTask) {
  return [
    ...
    (url) =>
      typeof url === "string" &&
      (isPublicMediaUrl(url) || /\.mp4(\?|#|$)/i.test(url))
  );
}
...
function readAxiosError(error: unknown, fallback: string) {
  ...
  if (
    ...
    return (
      readApiErrorMessage(responseData) ||
...
async function assertVideoBlob(blob: Blob) {
  ...
  if (typeof payload.code === "number" && payload.code !== 0)
    throw new Error(
      readApiErrorMessage(payload) || apiText("videoDownloadFailed")
    );
  if (payload.error?.message)
    throw new Error(
      readApiErrorMessage(payload.error.message) || payload.error.message
    );
}
