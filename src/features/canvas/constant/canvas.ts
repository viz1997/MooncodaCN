...
import { getNodeSpec as getRegistryNodeSpec } from "@/features/canvas/lib/canvas/node-registry";
import type { CanvasNodeMetadata } from "@/features/canvas/types/canvas";
...
export function getNodeSpec(type: string) {
  if ((Object.values(CanvasNodeType) as string[]).includes(type))
    return NODE_SPECS[type as CanvasNodeType];
