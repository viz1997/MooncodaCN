...
import {
  ...
  type PromptSource,
} from "@/features/canvas/services/api/prompt-source-presets";
...
const PROMPT_SOURCE_STORE_KEY = "mooncoda:canvas:prompt_source_store_v2";
  ...
  persist(
        ...
        set((state) => ({
          sources: state.sources.some((item) => item.id === source.id)
            ? state.sources.map((item) =>
                item.id === source.id && !item.builtIn
                  ? createPromptSource(source)
                  : item
              )
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        const savedSources = Array.isArray(persistedState.sources)
          ? persistedState.sources
          : [];
        const enabledById = new Map(
          savedSources.map((source) => [source.id, source.enabled])
        );
        const builtIn = DEFAULT_PROMPT_SOURCES.map((source) => ({
          ...source,
          enabled: enabledById.get(source.id) ?? source.enabled,
        }));
        const custom = savedSources
          .filter((source) => !source.builtIn)
          .map((source) => createPromptSource(source));
        return {
          ...current,
