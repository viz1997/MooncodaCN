...
const ASSET_STORE_KEY = "mooncoda:canvas:asset_store";
    ...
    parsed.state.assets = await Promise.all(
      parsed.state.assets.map(async (asset) => {
        ...
        if (asset.data.storageKey)
          return {
            ...asset,
            coverUrl: asset.coverUrl.startsWith("blob:")
              ? await resolveImageUrl(asset.data.storageKey, asset.coverUrl)
              : asset.coverUrl,
            data: {
              ...asset.data,
              dataUrl: await resolveImageUrl(
        ...
        return {
          ...asset,
          coverUrl: asset.coverUrl.startsWith("data:image/")
            ? image.url
            : asset.coverUrl,
          data: {
            ...asset.data,
      ...
      cleanupImages: (extra) => {
        window.setTimeout(async () => {
          const { useCanvasStore } = await import(
            "@/features/canvas/stores/canvas/use-canvas-store"
          );
