...
export const useCanvasUiStore = create<CanvasUiStore>((set) => ({
  ...
  selectedProjectIds: [],
  deleteProjectIds: [],
  startEditingProject: (editingProjectId, editingProjectTitle) =>
    set({ editingProjectId, editingProjectTitle }),
  ...
  toggleSelectedProjectId: (id, selected) =>
    set((state) => ({
      selectedProjectIds: selected
        ? [...new Set([...state.selectedProjectIds, id])]
        : state.selectedProjectIds.filter((item) => item !== id),
    })),
