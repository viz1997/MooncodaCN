...
type WorkbenchAgentStore = {
  ...
  dispatchImage: (
    command: Omit<WorkbenchCommand, "nonce" | "taskId">
  ) => string | undefined;
  dispatchVideo: (
    command: Omit<WorkbenchCommand, "nonce" | "taskId">
  ) => string | undefined;
  updateTask: (
    ...
    patch: Partial<
      Pick<
        ...
        "status" | "successCount" | "failCount" | "error"
      >
    >
  ) => void;
...
export const useWorkbenchAgentStore = create<WorkbenchAgentStore>((set) => ({
  ...
  dispatchImage: (command) => {
    ...
    const task = command.run
      ? createTask("image", commandNonce, command.prompt)
      : undefined;
    set((state) => ({
      imageCommand: { ...command, nonce: commandNonce, taskId: task?.id },
      tasks: task ? [task, ...state.tasks].slice(0, 30) : state.tasks,
    }));
    return task?.id;
  },
  dispatchVideo: (command) => {
    const commandNonce = nextNonce();
    const task = command.run
      ? createTask("video", commandNonce, command.prompt)
      : undefined;
    set((state) => ({
      videoCommand: { ...command, nonce: commandNonce, taskId: task?.id },
      tasks: task ? [task, ...state.tasks].slice(0, 30) : state.tasks,
    }));
    return task?.id;
  },
