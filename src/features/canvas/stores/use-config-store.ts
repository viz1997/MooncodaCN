...
export const CONFIG_STORE_KEY = "mooncoda:canvas:ai_config_store";
...
export const defaultConfig: AiConfig = {
  // 2026-08-20：V2 工作台默认走内置（后端代理 /api/canvas/generate，
  // mooncada 平台配的 wellapi key，不消耗积分）。旧用户 localStorage
  // 持久化的 channelMode="local" 仍优先（zustand persist 反序列化覆盖）。
  channelMode: "remote",
...
function findChannelModel(
  ...
  const channel = decoded
    ...
    : config.channels.find((item) =>
        item.models.some((model) => model.name === name)
      );
  const model = channel?.models.find((item) => item.name === name);
  return channel && model ? { channel, model } : null;
}

export function modelCapabilityOf(
  config: AiConfig,
  value: string
...
export function resolveModelForCapability(
  ...
  if (currentModel && modelMatchesCapability(config, currentModel, capability))
    return currentModel;
  if (defaultModel && modelMatchesCapability(config, defaultModel, capability))
    return defaultModel;
...
function isAiConfigReady(config: AiConfig, model: string) {
  ...
  // 内置渠道：后端用 env 系统 Key 调用，前端不需要 baseUrl/apiKey
  if (config.channelMode === "remote") return true;
  // 自定义渠道：必须 apiKey + baseUrl 都填好
...
export function useEffectiveConfig() {
  ...
  // channelMode 不再被强制覆盖为 "local"，由用户 / 持久化状态决定
  // —— Phase 3 内置渠道要靠 channelMode==="remote" 触发 /api/canvas/generate 代理
  return useMemo(() => config, [config]);
}

/** Normalize a mixed list of raw model names or model objects into deduped ChannelModel entries. */
export function normalizeChannelModels(
  models: Array<string | ChannelModel> | undefined
): ChannelModel[] {
  ...
  for (const item of models || []) {
    ...
    const capability =
      typeof item === "string"
        ? guessCapability(name)
        : item.capability || guessCapability(name);
    const script =
      typeof item === "string" ? undefined : item.script?.trim() || undefined;
...
export function modelOptionsFromChannels(channels: ModelChannel[]) {
  return uniqueModelOptions(
    channels.flatMap((channel) =>
      channel.models.map((model) => encodeChannelModel(channel.id, model.name))
    )
  );
}
...
export function normalizeModelOptionValue(
  ...
  if (decoded) {
    ...
    return channel && channel.models.some((item) => item.name === decoded.model)
      ? model
      : "";
  }
  const channel =
    channels.find((item) =>
      item.models.some((entry) => entry.name === model)
    ) || channels[0];
  return channel && channel.models.some((item) => item.name === model)
...
export function resolveModelChannel(config: AiConfig, value: string) {
  ...
  const matched = decoded
    ...
    : config.channels.find((channel) =>
        channel.models.some((item) => item.name === model)
      );
  return (
    matched ||
    config.channels[0] ||
  ...
  const channels = persistedChannels.map((channel, index) =>
    createModelChannel({
      ...
      name:
        channel.name ||
        (index === 0
          ? i18n.t("config.channels.defaultName")
          : i18n.t("config.channels.indexedName", { index: index + 1 })),
      models: normalizeChannelModels(channel.models),
    })
  );
  if (!channels.length) {
