...
import {
  ...
  fetchCodexSkills,
} from "@/features/canvas/services/api/canvas-agent";
...
export const useAgentSkillStore = create<AgentSkillStore>((set, get) => ({
  ...
  loadSkills: async (endpoint, token, forceReload = false) => {
    ...
    try {
      ...
      const autoPrompt = selectedSkill
        ? replaceAutoPrompt(
            current.autoPrompt,
            selectedSkill.interface?.defaultPrompt?.trim() ||
              `$${selectedSkill.name}`,
            false
          )
        : "";
      set({
        ...
        autoPrompt,
        selectionRevision:
          current.selectionRevision + (selectionChanged ? 1 : 0),
        loading: false,
        loaded: true,
        errors: (response.errors || []).map(skillErrorText),
      });
    } catch (error) {
      if (sequence !== loadSequence) return;
      set({
        ...
        errors: [
          error instanceof Error
            ? error.message
            : i18n.t("agent.state.skillReadFailed"),
        ],
      });
    }
  },
  selectSkill: (selectedSkill, prompt) => {
    ...
    const previousMarker = current.selectedSkill
      ? `$${current.selectedSkill.name}`
      : "";
    let nextPrompt = prompt;
    let autoPrompt = "";
    if (nextPrompt === undefined && selectedSkill) {
      const defaultPrompt =
        selectedSkill.interface?.defaultPrompt?.trim() || marker;
      if (
        !agent.prompt.trim() ||
        Boolean(current.autoPrompt && agent.prompt === current.autoPrompt)
      ) {
        ...
        nextPrompt = agent.prompt.includes(marker)
          ? agent.prompt
          : `${agent.prompt}${/\s$/.test(agent.prompt) ? "" : " "}${marker}`;
      }
    }
    if (nextPrompt !== undefined && agent.prompt !== nextPrompt)
      agent.setAgentState({ prompt: nextPrompt });
    set({
      selectedSkill,
      autoPrompt,
...
function clearSelectedSkillPrompt(
  selection: Pick<AgentSkillStore, "selectedSkill" | "autoPrompt">
) {
  const agent = useAgentStore.getState();
  ...
  if (agent.prompt.includes(marker))
    agent.setAgentState({ prompt: agent.prompt.replace(marker, "") });
}

function skillErrorText(value: unknown) {
  if (typeof value === "string") return value;
  if (!value || typeof value !== "object")
    return i18n.t("agent.state.skillParseFailed");
  ...
  const message =
    typeof item.error === "string"
      ...
      : typeof item.message === "string"
        ? item.message
        : i18n.t("agent.state.skillParseFailed");
