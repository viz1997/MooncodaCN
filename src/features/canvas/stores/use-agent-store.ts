...
import type {
  CanvasAgentOp,
  CanvasAgentSnapshot,
} from "@/features/canvas/lib/canvas/canvas-agent-ops";
...
export type AgentMessageAttachment = Pick<
  AgentAttachment,
  "id" | "name" | "url"
> &
  Partial<
    Pick<AgentAttachment, "type" | "size" | "width" | "height" | "dataUrl">
  >;
export type AgentCanvasReference = Pick<
  CanvasResourceReference,
  "nodeId" | "label" | "title" | "kind" | "previewUrl" | "text"
>;
...
export const useAgentStore = create<AgentStore>((set, get) => ({
  ...
  permissionMode:
    typeof window === "undefined"
      ...
      : (localStorage.getItem(
          "canvas-agent-permission-mode"
        ) as AgentPermissionMode) || "request",
  models: [],
  model:
    typeof window === "undefined"
      ? ""
      : localStorage.getItem("canvas-agent-model") || "",
  reasoningEffort:
    typeof window === "undefined"
      ? ""
      : (localStorage.getItem(
          "canvas-agent-reasoning-effort"
  ...
  connectAgent: (options) => {
    ...
    if (!endpoint || !token)
      return set({
        connectError: silent ? "" : i18n.t("agent.state.connectionRequired"),
      });
    try {
      ...
      return set({
        connectError: silent ? "" : i18n.t("agent.state.invalidUrl"),
      });
    }
  ...
  addEventLog: (item) =>
    set((state) => ({ eventLogs: [...state.eventLogs.slice(-160), item] })),
