...
import { ImageSettingsTheme } from "@/features/canvas/components/settings-panels/image-settings-panel";
...
const seedanceRatioLabelKeys: Record<string, string> = {
  "16:9": "landscape",
  "9:16": "portrait",
  "1:1": "square",
...
export const videoResolutionOptions = resolutionOptions.map((item) => ({
  value: item.value,
  label: item.label,
}));
      ...
      >
        {showTitle ? (
          <div className="text-lg font-semibold">
            {t("settingsPanels.video.title")}
          </div>
        ) : null}
        <SettingGroup
          title={t("settingsPanels.video.quality")}
        ...
        <SettingGroup
          title={t("settingsPanels.video.size")}
          color={theme.node.muted}
        >
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {sizeOptions.map((item) => (
              <button
                ...
                style={{
                  borderColor:
                    size === item.value ? theme.node.text : theme.node.stroke,
        ...
        <SettingGroup
          title={t("settingsPanels.video.seconds")}
          color={theme.node.muted}
        >
          <div className="grid grid-cols-3 gap-2.5">
  ...
  return (
    <ImageSettingsTheme theme={theme}>
      ...
      >
        ...
        <SettingGroup
          title={t("settingsPanels.video.resolution")}
          color={theme.node.muted}
        >
          <div className="grid grid-cols-3 gap-2.5">
        ...
        <SettingGroup
          title={t("settingsPanels.video.ratio")}
          color={theme.node.muted}
        >
          <div className="grid grid-cols-3 gap-2.5">
            {seedanceRatioOptions.map((item) => (
              <button
                ...
                style={{
                  borderColor:
                    ratio === item.value ? theme.node.text : theme.node.stroke,
                  color: theme.node.text,
                }}
                onMouseDown={(event) => event.stopPropagation()}
                onClick={() => onConfigChange("size", item.value)}
              >
                ...
                <span>
                  {i18n.t(
                    `settingsPanels.video.ratios.${seedanceRatioLabelKeys[item.value]}`
                  )}
                </span>
        ...
        <SettingGroup
          title={t("settingsPanels.video.duration")}
          color={theme.node.muted}
        >
        ...
        <SettingGroup
          title={t("settingsPanels.video.output")}
          color={theme.node.muted}
        >
          <div
          ...
          >
            ...
            />
            <SwitchRow
              label={t("settingsPanels.video.watermark")}
              checked={watermark}
              theme={theme}
              onChange={(checked) =>
...
export function videoSizeLabel(value: string) {
  ...
  if (value === "adaptive" || value === "auto")
    return i18n.t("settingsPanels.video.adaptive");
  if (ratio === value)
    return i18n.t(
      `settingsPanels.video.ratios.${seedanceRatioLabelKeys[ratio]}`
    );
  const size = normalizeVideoSizeValue(value);
  const option = sizeOptions.find((item) => item.value === size);
  return option
    ? i18n.t(`settingsPanels.video.sizes.${option.labelKey}`)
    : size;
}

export function videoSecondsLabel(value: string) {
  if (String(value).trim() === "-1")
    return i18n.t("settingsPanels.video.smart");
  return `${value || "6"}s`;
}
...
function ResolutionInput({
  ...
  return (
    ...
    >
      ...
      />
      <span
        className="grid w-7 place-items-center pr-1"
