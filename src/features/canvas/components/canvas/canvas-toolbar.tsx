...
export function CanvasToolbar({
  ...
  useNodeRegistryVersion();
  const extensionDefs = listNodeDefinitions().filter(
    (def) =>
      def.showInCreateMenu !== false && getNodePluginId(def.type) !== "builtin"
  );
  const dockStyle = {
  ...
  return (
    <div
      ref={rootRef}
      className="pointer-events-none absolute bottom-5 z-50 flex justify-center"
      style={{ left: 300, right: 16 }}
    >
      ...
      {extensionsOpen && extensionDefs.length ? (
        ...
        >
          <div className="px-1.5 pb-1.5 text-[11px] font-medium opacity-50">
            {t("canvas.toolbar.extensions")}
          </div>
          <div className="grid gap-0.5">
            {extensionDefs.map((def) => (
              ...
              >
                <span
                  className="grid size-7 shrink-0 place-items-center rounded-md text-base"
      ...
      {appearanceOpen ? (
        ...
        >
          <div className="px-1 pb-2 text-sm font-medium opacity-65">
            {t("canvas.toolbar.appearance")}
          </div>
          <div className="px-1 pb-1.5 text-[11px] font-medium opacity-50">
            {t("canvas.toolbar.themeMode")}
          </div>
          <div
          ...
          <div className="mt-3 px-1 pb-1.5 text-[11px] font-medium opacity-50">
          ...
          <Segmented
            ...
            onChange={(value) =>
              onBackgroundModeChange(value as CanvasBackgroundMode)
            }
            options={[
              {
                value: "dots",
      ...
      style={
        active
          ...
          : hovered === id && !disabled
            ? hoverStyle
            : {
                color: danger ? "#f87171" : theme.toolbar.item,
...
function CanvasThemeButton({
  ...
  const { t } = useTranslation();
  const label =
    targetTheme === "dark" ? t("topNav.darkTheme") : t("topNav.lightTheme");
...
function DockTip({
  ...
  return (
    <span
      className="absolute bottom-[calc(100%+8px)] -translate-x-1/2 rounded-md px-2 py-1 text-xs shadow-lg"
      style={{ left: x, background: theme.node.text, color: theme.node.panel }}
    >
      {label}
    </span>
  );
...
function getTipX(wrap: HTMLDivElement | null, target: HTMLElement) {
  if (!wrap) return 0;
  const wrapBox =
    wrap.parentElement?.getBoundingClientRect() || wrap.getBoundingClientRect();
