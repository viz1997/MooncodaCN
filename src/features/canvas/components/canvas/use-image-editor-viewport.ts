...
export function useImageEditorViewport(image: ImageSize | null, open: boolean) {
  ...
  useLayoutEffect(() => {
    ...
    const nextLeft = Math.max(
      0,
      (Math.max(viewport.clientWidth, nextWidth) - nextWidth) / 2
    );
    const nextTop = Math.max(
      0,
      (Math.max(viewport.clientHeight, nextHeight) - nextHeight) / 2
    );
    viewport.scrollLeft =
      nextLeft + anchor.ratioX * nextWidth - anchor.viewportX;
    viewport.scrollTop =
      nextTop + anchor.ratioY * nextHeight - anchor.viewportY;
    zoomAnchorRef.current = null;
  ...
  const setZoomAround = useCallback(
    (nextZoom: number, clientX?: number, clientY?: number) => {
      ...
      const ratioX = clamp(
        (pointerX - stageRect.left) / Math.max(1, stageRect.width),
        0,
        1
      );
      const ratioY = clamp(
        (pointerY - stageRect.top) / Math.max(1, stageRect.height),
        0,
        1
      );
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      event.stopPropagation();
      setZoomAround(
        event.deltaY < 0 ? zoom * zoomStep : zoom / zoomStep,
        event.clientX,
        event.clientY
      );
    };
  ...
  const preventAuxClick = useCallback(
  ...
  return {
    ...
    spacePressed,
    scrollClassName:
      zoom > minZoom + 0.001 ? "overflow-scroll" : "overflow-hidden",
    panHandlers: {
