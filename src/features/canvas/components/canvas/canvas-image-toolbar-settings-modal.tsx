...
export function ImageToolSettingsModal({
  ...
  const [previewScroll, setPreviewScroll] = useState<PreviewScroll>({
    left: 0,
    max: 0,
    viewport: 1,
    content: 1,
  ...
  const scrollbarWidth =
    scrollbarTrackRef.current?.clientWidth || previewScroll.viewport;
  const scrollbarThumbWidth =
    previewScroll.max > 0
      ? Math.min(
          ...
          Math.max(
            64,
            (previewScroll.viewport / previewScroll.content) * scrollbarWidth
          )
        )
      : scrollbarWidth;
      ...
      >
        <div className="relative flex min-h-[300px] w-full justify-center pt-20 pb-9">
          ...
          <input
            ...
            style={
              {
                "--preview-scrollbar-thumb-width": `${scrollbarThumbWidth}px`,
              } as CSSProperties
            }
            onInput={(event) =>
              setPreviewScrollLeft(Number(event.currentTarget.value))
            }
            onChange={(event) =>
              setPreviewScrollLeft(Number(event.target.value))
            }
          />
        </div>
      </Card>
      ...
      <Form layout="vertical" className="!mb-0">
        ...
        >
          <Checkbox.Group
            ...
            onChange={(values) =>
              updateSelectedTools(values as ImageQuickToolId[])
            }
          >
            {tools.map((tool) => (
...
function PreviewToolbarItem({
  ...
  return (
    <Tooltip title={tool.title}>
      <span
        className="flex h-12 shrink-0 items-center px-1.5"
        style={{ color: tool.danger ? "#ef4444" : undefined }}
      >
        <span
          className={`flex h-9 items-center rounded-lg px-2 ${showLabels ? "gap-2" : "justify-center"}`}
        >
          {tool.icon}
          {showLabels ? (
