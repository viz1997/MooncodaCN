...
import type { CanvasResourceReference } from "@/features/canvas/lib/canvas/canvas-resource-references";
import { getNodeDefinition } from "@/features/canvas/lib/canvas/node-registry";
import { buildNodeContext } from "@/features/canvas/lib/canvas/plugin-node-context";
import { canvasThemes } from "@/features/canvas/lib/canvas-theme";
import { formatBytes } from "@/features/canvas/lib/image-utils";
...
export const CanvasNode = React.memo(function CanvasNode({
  ...
  const pluginContext = useMemo<CanvasNodeContext | null>(
    () =>
      pluginHost
        ? buildNodeContext(pluginHost, data, theme, scale, isSelected)
        : null,
    [pluginHost, data, theme, scale, isSelected]
  );
  const [isEditingContent, setIsEditingContent] = useState(false);
  const [isEditingTitle, setIsEditingTitle] = useState(false);
  const [titleDraft, setTitleDraft] = useState(data.title || "");
  const hasImageContent =
    data.type === CanvasNodeType.Image && Boolean(data.metadata?.content);
  const hasVideoContent =
    data.type === CanvasNodeType.Video && Boolean(data.metadata?.content);
  const hasAudioContent =
    data.type === CanvasNodeType.Audio && Boolean(data.metadata?.content);
  const isGroup = data.type === CanvasNodeType.Group;
  const batchCount =
    data.type === CanvasNodeType.Image ? data.metadata?.images?.length || 0 : 0;
  ...
  const forceInteractive = supportsInteractionToggle
    ? Boolean(definition?.forceInteractive?.(data))
    : false;
  const contentInteractive =
    !supportsInteractionToggle || forceInteractive || !data.metadata?.content
      ? true
      : Boolean(data.metadata?.interactive);
  ...
  const imageBorderColor = isActive
    ? selectionBlue
    : isRelated
      ? theme.node.muted
      : "transparent";
  ...
  useEffect(() => {
    ...
    const handleOutsidePointerDown = (event: PointerEvent) => {
      ...
      if (target instanceof Node && titleInputRef.current?.contains(target))
        return;
      finishTitleEditing();
    };
  ...
  const handleResizeMove = useCallback(
    (event: MouseEvent) => {
      ...
      const minHeight = 160;
      const startRight =
        resizeRef.current.startLeft + resizeRef.current.startWidth;
      const startBottom =
        resizeRef.current.startTop + resizeRef.current.startHeight;
      const fromLeft = resizeRef.current.corner.includes("left");
      const fromTop = resizeRef.current.corner.includes("top");
      const rawWidth = Math.max(
        minWidth,
        resizeRef.current.startWidth + (fromLeft ? -dx : dx)
      );
      const rawHeight = Math.max(
        minHeight,
        resizeRef.current.startHeight + (fromTop ? -dy : dy)
      );
      let width = rawWidth;
  ...
  const handleResizeMouseDown = (
    ...
    resizeRef.current = {
      ...
      keepRatio:
        (data.type === CanvasNodeType.Image && !data.metadata?.freeResize) ||
        data.type === CanvasNodeType.Video ||
        Boolean(definition?.keepAspectRatio?.(data)),
      ratio:
        (data.metadata?.naturalWidth || data.width) /
        (data.metadata?.naturalHeight || data.height || 1),
    };
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: isGroup
            ? `${theme.toolbar.panel}66`
            : hasImageContent || hasVideoContent || transparentBg
              ? "transparent"
              : theme.node.fill,
          borderColor: isGroup
      ...
      >
        ...
        >
          <NodeContent
            ...
            onSetBatchPrimary={(imageId) =>
              onSetBatchPrimary?.(data.id, imageId)
            }
            onDuplicateBatchImage={(imageId) =>
              onDuplicateBatchImage?.(data, imageId)
            }
            onDownloadBatchImage={(imageId) =>
              onDownloadBatchImage?.(data, imageId)
            }
            onRetryBatchImage={(imageId) => onRetryBatchImage?.(data, imageId)}
            onDeleteBatchImage={(imageId) =>
              onDeleteBatchImage?.(data.id, imageId)
            }
        ...
        !hasAudioContent ? (
          <div
            ...
            style={{
              background: `linear-gradient(to top, ${theme.canvas.background}66, transparent)`,
            }}
          />
        ) : null}
      ...
      {!isGroup ? (
        <ConnectionHandleDot
          side="right"
          visible={
            (definition?.hasSourceHandle ?? true) &&
      ...
      {showPanel && !isGroup && renderPanel ? (
        <div className="absolute left-1/2 top-full z-[70] w-[600px] -translate-x-1/2 pt-4">
          {renderPanel(data)}
        </div>
      ) : null}
    </div>
...
function NodeContent(props: NodeContentRendererProps) {
  if (props.node.type === CanvasNodeType.Config && props.renderNodeContent)
    return props.renderNodeContent(props.node);
...
const nodeContentRenderers = {
  ...
  CanvasNodeType,
  (props: NodeContentRendererProps) => ReactNode
>;
  ...
  return (
    <div className="pointer-events-none flex h-full w-full flex-col p-4">
      ...
      >
        ...
        <span
          className="ml-auto rounded-full px-2 py-1 text-[11px] font-medium"
      ...
      <div
        className="mt-3 flex-1 rounded-2xl border border-dashed"
        style={{
          borderColor: theme.node.stroke,
          background: `${theme.node.fill}55`,
        }}
      />
    </div>
  );
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          borderColor: theme.node.stroke,
          borderTopColor: theme.node.activeStroke,
        }}
      />
  ...
  return (
    <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
      <div className="text-xs leading-5 text-red-300">
        {node.metadata?.errorDetails || t("canvas.node.failed")}
      </div>
      <button
        type="button"
...
function MissingPluginContent({
  theme,
  type,
}: Pick<NodeContentRendererProps, "theme"> & { type: string }) {
  const { t } = useTranslation();
  return (
    <div
    ...
    >
      ...
      <span className="text-[11px] opacity-70">
        {t("canvas.node.missingPluginDescription", { type })}
      </span>
    </div>
  );
}
      ...
      {isEditingContent ? (
        ...
        >
          {node.metadata?.content || (
            <span style={{ color: theme.node.placeholder }}>
...
function EmptyImageContent({ theme }: NodeContentRendererProps) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[10px] tracking-[0.18em] opacity-50">
...
function ImageContent({
  ...
  return (
    ...
    >
      ...
      {primaryImage?.status === "error" ? (
        <BatchImageFailureActions
          placement="left"
          onRetry={() => onRetryBatchImage?.(primaryImage.id)}
          onDelete={() => onDeleteBatchImage?.(primaryImage.id)}
        />
      ) : null}
      {primaryImage?.content ? (
        <button
          type="button"
          className="absolute left-2.5 top-2.5 z-30 flex h-8 items-center gap-1 rounded-lg border px-2 text-[10px] font-medium shadow-[0_6px_18px_rgba(15,23,42,.16)] backdrop-blur-md transition hover:scale-[1.02]"
          style={{
            background: theme.toolbar.panel,
            borderColor: theme.toolbar.border,
          ...
          onClick={(event) => (
            event.stopPropagation(), onDownloadBatchImage?.(primaryImage.id)
          )}
        >
      ...
      {isBatchRoot ? (
        <button
          ...
          aria-label={
            batchExpanded
              ? t("canvas.node.batchExpanded")
              : t("canvas.node.batchCollapsed")
          }
          onClick={(event) => {
            event.stopPropagation();
            onToggleBatch?.();
        ...
        >
          <span className="leading-none">
            {t("canvas.controls.images", { count: batchCount })}
          </span>
          <ChevronRight
            className={`size-3.5 opacity-80 transition-transform ${batchExpanded ? "rotate-90" : ""}`}
          />
        </button>
      ) : null}
    </BatchFrame>
...
function BatchImageFailureActions({
  placement,
  onRetry,
  onDelete,
}: {
  ...
  return (
    <div
      className={`absolute top-3 z-30 flex items-center gap-1.5 ${placement === "left" ? "left-3" : "right-3"}`}
    >
      <button
        type="button"
  ...
  return (
    <div
      ...
      style={{
        ...
        color: failed ? theme.node.text : theme.node.activeStroke,
      }}
    >
      {failed ? (
        <span className="text-xs leading-5">
          {image.errorDetails || t("canvas.node.failed")}
        </span>
      ) : (
        <div
          className="size-10 animate-spin rounded-full border-2"
    ...
    >
      {isBatchRoot ? (
        <div className="pointer-events-none absolute inset-0 overflow-visible">
          {Array.from({ length: Math.min(batchCount - 1, 3) }).map(
            (_, index) => (
              <div
                key={index}
