...
import { useCopyText } from "@/features/canvas/hooks/use-copy-text";
...
import type { CanvasNodeToolbarItem } from "@/features/canvas/types/canvas-plugin";
import {
  type ImageToolbarSettingsTool,
  ImageToolSettingsModal,
} from "./canvas-image-toolbar-settings-modal";
import {
...
export function CanvasNodeHoverToolbar({
  ...
  const [quickImageToolIds, setQuickImageToolIds] = useState<
    ImageQuickToolId[]
  >(defaultImageQuickToolIds);
  const [showImageToolLabels, setShowImageToolLabels] = useState(true);
  const [draftImageToolIds, setDraftImageToolIds] = useState<
    ImageQuickToolId[]
  >(defaultImageQuickToolIds);
  const [draftShowImageToolLabels, setDraftShowImageToolLabels] =
    useState(true);
      ...
      ? [
          {
            ...
            title: t(
              hasVideo
                ? "canvas.nodeToolbar.replaceVideo"
                : "canvas.nodeToolbar.uploadVideo"
            ),
            label: t(
              hasVideo
                ? "canvas.nodeToolbar.replaceVideo"
      ...
      ? [
          {
            ...
            title: t(
              hasAudio
                ? "canvas.nodeToolbar.replaceAudio"
                : "canvas.nodeToolbar.uploadAudio"
            ),
            label: t(
              hasAudio
                ? "canvas.nodeToolbar.replaceAudio"
  ...
  const toolbarTools = hasImage
    ? [...baseToolbarTools, ...nodeToolbarTools].filter((tool) =>
        quickImageToolIdSet.has(tool.id as ImageQuickToolId)
      )
    : [...baseToolbarTools, ...nodeToolbarTools, ...extraTools];
  const selectableImageToolbarTools = [
    ...baseToolbarTools,
    ...nodeToolbarTools,
  ].filter((tool) => tool.id !== "retry") as ImageToolbarSettingsTool[];
  ...
  const setDraftImageToolVisible = (id: ImageQuickToolId, visible: boolean) => {
    setDraftImageToolIds((current) => {
      ...
      return selectableImageToolbarTools
...
export function CanvasNodeInfoModal({
  ...
  const imageBytes =
    node?.type === CanvasNodeType.Image && node.metadata?.content
      ? getDataUrlByteSize(node.metadata.content)
      : 0;
  const batchCount =
    node?.type === CanvasNodeType.Image
      ? node.metadata?.images?.length || 0
      : 0;
  const json = useMemo(() => {
    if (!node) return "";
  ...
  return (
    ...
    >
      {node ? (
        <div
          className="h-[56vh] min-h-[360px] select-text text-sm"
          ...
          {view === "info" ? (
            <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
              ...
              <InfoRow
                ...
                value={node.title || t("canvas.node.untitled")}
              />
              <InfoRow
              ...
              <InfoRow
                ...
                value={node.metadata?.status || "idle"}
              />
              {batchCount > 1 ? (
                <InfoRow
              ...
              {imageBytes ? (
                <InfoRow
                  label={t("canvas.nodeToolbar.imageSize")}
...
function ToolbarAction({
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`flex h-9 items-center ${hasText ? "gap-2 px-2.5" : "justify-center px-2"} rounded-lg transition group-hover:bg-[#f0f0f1] ${active ? "bg-[#eeeeef]" : ""}`}
        >
          {icon}
