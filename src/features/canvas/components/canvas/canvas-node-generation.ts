...
import { getGenerationResourceNodes } from "@/features/canvas/lib/canvas/canvas-resource-references";
...
import type { AiTextMessage } from "@/features/canvas/services/api/image";
import {
...
export function buildNodeGenerationInputs(
  ...
  return getGenerationResourceNodes(nodeId, nodes, connections).flatMap(
    (node): NodeGenerationInput[] => {
      ...
      if (image)
        return [
          { nodeId: node.id, type: "image" as const, title: node.title, image },
        ];
      const video = readReferenceVideo(node);
      if (video)
        return [
          { nodeId: node.id, type: "video" as const, title: node.title, video },
        ];
      const audio = readReferenceAudio(node);
      if (audio)
        return [
          { nodeId: node.id, type: "audio" as const, title: node.title, audio },
        ];
      const text = readNodeTextInput(node);
      if (text)
        return [
          { nodeId: node.id, type: "text" as const, title: node.title, text },
        ];
      return [];
    }
  );
...
export async function hydrateNodeGenerationContext(
  ...
  const { imageToDataUrl } = await import(
    "@/features/canvas/services/image-storage"
  );
  return {
    ...context,
