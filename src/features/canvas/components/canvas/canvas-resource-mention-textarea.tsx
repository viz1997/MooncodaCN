  ...
  const activeLabels = useMemo(
    () =>
      highlightLabels
        ? Array.from(
            new Set(
              references.filter((item) => item.active).map((item) => item.label)
            )
    ...
    <div className={`relative h-full w-full ${containerClassName || ""}`}>
      {showOverlay ? (
        <div
          ...
          style={{ ...style, color: theme.node.text }}
        >
          <MentionHighlightText
            value={value || props.placeholder?.toString() || ""}
            labels={activeLabels}
            placeholder={!value}
          />
        </div>
        ...
        onKeyDown={(event) => {
          ...
          if (
            ...
            if (el && el.selectionStart === el.selectionEnd) {
              const result = deleteAdjacentLabel(
                value,
                el.selectionStart,
    ...
    <>
      {value.split(pattern).map((part, index) =>
        labels.includes(part) ? (
          <span
            key={`${part}-${index}`}
            className="rounded-md bg-[#2f80ff]/16 px-1 py-0.5 font-medium text-[#2f80ff] ring-1 ring-[#2f80ff]/24"
          >
            {part}
          </span>
        ) : (
...
function MentionMenu({
  ...
  const showAbove =
    caretTop + lineHeight + gap + maxMenuHeight > boundary.bottom &&
    caretTop - gap - maxMenuHeight >= boundary.top;
  const top = clamp(
    showAbove ? caretTop - gap - maxMenuHeight : caretTop + lineHeight + gap,
    boundary.top + 8,
    boundary.bottom - maxMenuHeight - 8
  );
...
function deleteAdjacentLabel(
  ...
  if (direction === "backward") {
    ...
    for (const label of labels) {
      if (!label) continue;
      const match = new RegExp(`(^|\\s)(${escapeRegExp(label)})\\s*$`).exec(
        before
      );
      if (match) {
    ...
    for (const label of labels) {
      ...
      if (match) {
        return {
          value: value.slice(0, caret) + value.slice(caret + match[0].length),
          caret,
        };
      }
    }
