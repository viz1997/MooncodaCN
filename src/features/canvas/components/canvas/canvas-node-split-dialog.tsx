...
import { useImageEditorViewport } from "@/features/canvas/components/canvas/use-image-editor-viewport";
import type { ImageSplitParams } from "@/features/canvas/lib/canvas/canvas-image-data";
...
export function CanvasNodeSplitDialog({
  ...
  const update = (key: "rows" | "columns", value: string | number | null) => {
    ...
    setParams((current) => ({
      ...current,
      [key]: count,
      [key === "rows" ? "horizontalLines" : "verticalLines"]:
        buildGridLines(count),
    }));
  };
  const addLine = (axis: "horizontal" | "vertical") => {
    ...
    setParams({
      ...params,
      [key]: lines,
      rows: axis === "horizontal" ? lines.length + 1 : params.rows,
      columns: axis === "vertical" ? lines.length + 1 : params.columns,
    });
  };
  const deleteLine = () => {
    if (!active) return;
    ...
    setParams((current) => {
      const key =
        active.axis === "horizontal" ? "horizontalLines" : "verticalLines";
      const lines = (current[key] || []).filter(
        (_, index) => index !== active.index
      );
      return {
        ...current,
        [key]: lines,
        rows: active.axis === "horizontal" ? lines.length + 1 : current.rows,
        columns:
          active.axis === "vertical" ? lines.length + 1 : current.columns,
      };
    });
    setActive(null);
  };
  ...
  const resetLines = () => {
    ...
    setParams((current) => ({
      ...current,
      horizontalLines: buildGridLines(current.rows),
      verticalLines: buildGridLines(current.columns),
    }));
  };
      ...
      <div className="space-y-5" data-canvas-no-zoom>
        ...
        <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_280px]">
          <div className="rounded-xl border p-4">
            ...
            >
              <div className="relative" style={viewport.contentStyle}>
                <div
                  ref={previewRef}
                  className="absolute isolate overflow-hidden rounded-lg bg-black [backface-visibility:hidden] [contain:layout_paint] [transform:translateZ(0)]"
              ...
              <div className="flex items-center gap-1">
                <Tooltip title={t("canvas.editors.undoSplitTitle")}>
                  <Button
                    ...
                    aria-label={t("canvas.editors.undoSplit")}
                    onClick={undoSplit}
                  />
                </Tooltip>
                <Tooltip title={t("canvas.editors.redoSplitTitle")}>
                  <Button
                    ...
                    disabled={!redoSize}
                    aria-label={t("canvas.editors.redoSplit")}
                    onClick={redoSplit}
                  />
                </Tooltip>
                <Tooltip title={t("canvas.editors.zoomOut")}>
                  <Button
                    ...
                    aria-label={t("canvas.editors.zoomOut")}
                    onClick={viewport.zoomOut}
                  />
                </Tooltip>
                <button
                  type="button"
                  className="min-w-14 text-center text-xs font-semibold tabular-nums opacity-70"
                ...
                <Tooltip title={t("canvas.editors.zoomIn")}>
                  <Button
                    ...
                    aria-label={t("canvas.editors.zoomIn")}
          ...
          <div className="space-y-5 py-2">
            <NumberField
              label={t("canvas.editors.rows")}
              value={rows}
              onChange={(value) => update("rows", value)}
            />
            <NumberField
              label={t("canvas.editors.columns")}
              value={columns}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button
                icon={<PanelTop className="size-4 rotate-90" />}
            ...
            <div className="rounded-xl border px-4 py-3 text-sm">
              ...
              <div className="mt-2 flex items-center justify-between">
                <span className="opacity-60">
                  {t("canvas.editors.averageSize")}
                </span>
                <span className="font-semibold">
                  {pieceSize
                    ? `${pieceSize.width} x ${pieceSize.height}`
                    : t("canvas.editors.unknown")}
                </span>
              </div>
            </div>
            <Button
  ...
  return (
    <div className="pointer-events-none absolute inset-0">
      {verticalLines.map((line, index) => (
        <div
          key={`column-${index}`}
          className="pointer-events-auto absolute inset-y-0 -ml-2 w-4 cursor-ew-resize"
          style={{ left: `${line * 100}%` }}
          onPointerDown={(event) => onPointerDown("vertical", index, event)}
        >
          <div
            className={`absolute left-1/2 top-0 h-full border-l shadow-[0_0_0_1px_rgba(0,0,0,.35)] ${active?.axis === "vertical" && active.index === index ? "border-amber-300" : "border-white/90"}`}
          />
        </div>
      ))}
      {horizontalLines.map((line, index) => (
        <div
          key={`row-${index}`}
          className="pointer-events-auto absolute inset-x-0 -mt-2 h-4 cursor-ns-resize"
          style={{ top: `${line * 100}%` }}
          onPointerDown={(event) => onPointerDown("horizontal", index, event)}
        >
          <div
            className={`absolute left-0 top-1/2 w-full border-t shadow-[0_0_0_1px_rgba(0,0,0,.35)] ${active?.axis === "horizontal" && active.index === index ? "border-amber-300" : "border-white/90"}`}
          />
        </div>
      ))}
    </div>
...
function pushHistory(
  historyRef: { current: CanvasImageSplitParams[] },
  redoRef: { current: CanvasImageSplitParams[] },
  params: CanvasImageSplitParams,
  setHistorySize: (size: number) => void,
  setRedoSize: (size: number) => void
) {
  historyRef.current.push(cloneSplitParams(params));
