...
import {
  ...
  uninstallPlugin,
  updatePlugin,
} from "@/features/canvas/lib/canvas/plugin-loader";
import {
  fetchOfficialPlugins,
  hasUpgrade,
  type OfficialPluginEntry,
} from "@/features/canvas/lib/canvas/plugin-registry";
...
export function CanvasPluginManagerModal({
  open,
  onClose,
}: {
  open: boolean;
  ...
  const recordById = useMemo(
    () => new Map(plugins.map((item) => [item.id, item])),
    [plugins]
  );
  const localPlugins = useMemo(
    () => plugins.filter((item) => item.local),
    [plugins]
  );
  const thirdPartyPlugins = useMemo(
    () => plugins.filter((item) => !item.local && !item.official),
    [plugins]
  );
  ...
  useEffect(() => {
    if (open && official.length === 0 && !loadingOfficial && !officialError)
      void loadOfficial();
  ...
  const handleInstallUrl = async () => {
    ...
    try {
      ...
      message.success(
        t("canvas.plugins.installedPlugin", { name: plugin.name })
      );
      setUrl("");
    } catch (error) {
      message.error(
        t("canvas.plugins.installFailed", {
    ...
    <>
      <Switch
        ...
        onChange={(checked) =>
          runOnPlugin(
            record,
            () => setPluginEnabled(record, checked),
            t(checked ? "canvas.plugins.enabled" : "canvas.plugins.disabled")
          )
        }
      />
      {!record.local && (
        <>
          <Button
            ...
            title={t(
              upgradable
                ? "canvas.plugins.upgradeAvailable"
                : "canvas.plugins.updateFromSource"
            )}
            onClick={() =>
              runOnPlugin(
                record,
                async () => void (await updatePlugin(record)),
                t("canvas.plugins.updated")
              )
            }
          />
          <Popconfirm
            title={t("canvas.plugins.uninstallTitle")}
            okText={t("canvas.plugins.uninstall")}
            cancelText={t("canvas.editors.cancel")}
            onConfirm={() => uninstallPlugin(record.id)}
          >
            <Button
              type="text"
              size="small"
              ...
              title={t("canvas.plugins.uninstall")}
            />
          </Popconfirm>
        </>
      )}
  ...
  const withUpgradeDot = (icon: ReactNode) => (
    <span className="relative inline-flex">
      ...
      <span
        ...
        style={{
          background: "#22c55e",
          boxShadow: `0 0 0 2px ${theme.node.fill}`,
        }}
        title={t("canvas.plugins.newVersion")}
      />
    </span>
  );
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          icon={
            <RefreshCw
              className={`size-4 ${loadingOfficial ? "animate-spin" : ""}`}
            />
          }
        ...
        <div className="thin-scrollbar max-h-[46vh] space-y-2 overflow-auto">
          {official.map((entry) => {
            ...
            const upgradable = Boolean(
              record && hasUpgrade(record.version, entry.version)
            );
  ...
  const thirdPartyTab = (
    <div className="space-y-3">
      <div className="flex gap-2">
        <Input
          placeholder={t("canvas.plugins.urlPlaceholder")}
          value={url}
  ...
  const tabs = [
    {
      key: "official",
      label: t("canvas.plugins.official"),
      children: officialTab,
    },
    ...(localPlugins.length > 0
      ? [{ key: "local", label: t("canvas.plugins.local"), children: localTab }]
      : []),
    {
      key: "third",
      label: t("canvas.plugins.thirdParty"),
