...
import type { CanvasNodeData } from "@/features/canvas/types/canvas";
...
const defaultBaseToolIds: ImageQuickToolId[] = [
  "info",
  "delete",
  "saveAsset",
  "download",
    ...
    label: (node) =>
      i18n.t(
        node.metadata?.freeResize
          ? "canvas.imageTools.free"
          : "canvas.imageTools.locked"
      ),
    title: (node) =>
      i18n.t(
        node.metadata?.freeResize
          ? "canvas.imageTools.lockTitle"
          : "canvas.imageTools.freeTitle"
      ),
    icon: (node) =>
      node.metadata?.freeResize ? (
...
export function buildImageToolbarTools(
  node: CanvasNodeData,
...
function resolveToolText(
  value: string | ((node: CanvasNodeData) => string),
  node: CanvasNodeData
) {
