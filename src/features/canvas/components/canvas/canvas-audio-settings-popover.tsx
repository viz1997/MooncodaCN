...
import { AudioSettingsPanel } from "@/features/canvas/components/settings-panels/audio-settings-panel";
import {
  audioFormatLabel,
  audioSpeedLabel,
  audioVoiceLabel,
    ...
    <>
      <span ref={buttonRef} className="inline-flex min-w-0">
        <Button
          ...
          className={
            buttonClassName ||
            "!h-8 !max-w-[170px] !justify-start !rounded-full !px-2.5"
          }
        ...
        >
          <span className="truncate">
            {audioVoiceLabel(config.audioVoice)} ·{" "}
            {audioFormatLabel(config.audioFormat)} ·{" "}
            {audioSpeedLabel(config.audioSpeed)}
          </span>
        </Button>
      </span>
      {panel}
  ...
  const style = {
    ...
    ...(topPlacement
      ...
      : {
          ...
          maxHeight: Math.max(
            260,
            window.innerHeight - buttonRect.bottom - margin * 2
          ),
        }),
