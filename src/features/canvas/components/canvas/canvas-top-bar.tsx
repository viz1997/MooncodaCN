...
import { UserStatusActions } from "@/features/canvas/components/layout/user-status-actions";
import { DOCS_URL } from "@/features/canvas/constant/env";
import { canvasThemes } from "@/features/canvas/lib/canvas-theme";
import { useCanvasSidePanelStore } from "@/features/canvas/stores/use-canvas-side-panel-store";
  ...
  return (
    <>
      <div className="pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-16 items-center justify-between pl-1 pr-4">
        <div className="pointer-events-auto flex min-w-0 items-center gap-2">
          <Tooltip
            title={
              sidePanelOpen
                ? t("canvas.collapsePanel")
                : t("canvas.expandPanel")
            }
          >
            <button
              type="button"
            ...
            menu={{
              items: [
                ...
                },
                {
                  ...
                  label: (
                    <MenuLabel text={t("canvas.redo")} shortcut="⌘ ⇧ Z / ⌘ Y" />
                  ),
                  onClick: onRedo,
                },
              ],
      ...
      >
        ...
        >
          <Shortcut
            keys={["Ctrl / Space", t("canvas.shortcut.drag")]}
            value={t("canvas.shortcut.toggleTool")}
          />
          <Shortcut
            keys={[t("canvas.shortcut.wheel")]}
            value={t("canvas.shortcut.zoom")}
          />
          <Shortcut
            keys={[t("canvas.shortcut.zoomSlider")]}
            value={t("canvas.shortcut.preciseZoom")}
          />
          <Shortcut
            keys={[t("canvas.shortcut.drag")]}
            value={t("canvas.shortcut.boxSelect")}
          />
          <Shortcut
            keys={["Shift / Cmd", t("canvas.shortcut.click")]}
            value={t("canvas.shortcut.addSelection")}
          />
          <Shortcut
            keys={["Ctrl / Cmd", "A"]}
            value={t("canvas.shortcut.selectAll")}
          />
          <Shortcut
            keys={["Ctrl / Cmd", "C / V"]}
            value={t("canvas.shortcut.copyPaste")}
          />
          ...
          <Shortcut
            ...
            value={t("canvas.shortcut.delete")}
          />
          <Shortcut keys={["Esc"]} value={t("canvas.shortcut.escape")} />
          <Shortcut
            keys={[t("canvas.shortcut.dropMedia")]}
            value={t("canvas.shortcut.upload")}
          />
        </div>
      </Modal>
    </>
...
function CompactAgentStatus({
  ...
  const label = status.connected
    ? t("canvas.agentConnected")
    : status.enabled
      ? t("canvas.agentConnecting", {
          activity: status.activity || t("canvas.connecting"),
        })
      : t("canvas.agentDisconnected");
