...
import {
  isImeComposing,
  isPlainEnterKey,
} from "@/features/canvas/lib/keyboard-event";
...
export function CanvasPromptChipInput({
  value,
  references,
  onChange,
  onSubmit,
  ...
  const activeReferences = useMemo(
    () => references.filter((item) => item.active),
    [references]
  );
  const referenceByLabel = useMemo(
    () => new Map(activeReferences.map((item) => [item.label, item])),
    [activeReferences]
  );
  // Match longer labels first so a shorter label cannot split a longer one.
  const activeLabels = useMemo(
    () =>
      Array.from(new Set(activeReferences.map((item) => item.label))).sort(
        (a, b) => b.length - a.length
      ),
    [activeReferences]
  );
  const tokens = useMemo(
    () => parseTokens(value, activeLabels),
    [value, activeLabels]
  );
  ...
  const candidates = useMemo(() => {
    ...
    return activeReferences.filter((item) =>
      `${item.label} ${item.title} ${item.kind} ${item.text || ""}`
        .toLowerCase()
        .includes(query)
    );
  ...
  return (
    <div className="relative w-full">
      {showPlaceholder && placeholder ? (
        <div
          className="pointer-events-none absolute left-3 top-2 text-sm leading-5"
      ...
      {imagePreview ? (
        <Image
          src={imagePreview}
          alt={i18n.t("canvas.composer.imagePreview")}
          style={{ display: "none" }}
          preview={{
            visible: true,
            src: imagePreview,
...
function MentionMenu({
  ...
  const showAbove =
    anchor.bottom + gap + maxMenuHeight > window.innerHeight &&
    anchor.top - gap - maxMenuHeight >= 0;
  const top = showAbove
    ? anchor.top - gap - maxMenuHeight
    : anchor.bottom + gap;
    ...
    >
      {references.map((reference, index) => (
        <button
          ...
          style={{
            background:
              index === activeIndex ? theme.toolbar.activeBg : "transparent",
            color:
              index === activeIndex
...
function adjacentReferenceNode(range: Range, key: string) {
  ...
  if (container.nodeType === Node.TEXT_NODE) {
    ...
    if ((previous && offset > 0) || (!previous && offset < text.length))
      return null;
...
function findReferenceSibling(
  ...
  return current instanceof HTMLElement && current.dataset.refLabel
    ? current
    : null;
}
