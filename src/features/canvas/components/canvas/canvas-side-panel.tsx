...
import { PromptDetailDialog } from "@/features/canvas/components/prompts/prompt-detail-dialog";
import { exportCanvasNodes } from "@/features/canvas/lib/canvas/canvas-export";
import { getNodeDefinition } from "@/features/canvas/lib/canvas/node-registry";
import {
  type CanvasTheme,
  canvasThemes,
...
import {
  fetchSourcePrompts,
  type Prompt,
} from "@/features/canvas/services/api/prompts";
...
} from "@/features/canvas/stores/use-canvas-side-panel-store";
import { usePromptSourceStore } from "@/features/canvas/stores/use-prompt-source-store";
...
/**
 * Native HTML <select> 替代 antd 6 <Select>：
 *
 * 背景：antd 6.6.1 + React 19 在画布 CanvasNodesTab 渲染时反复触发
 * "Maximum update depth exceeded"，栈底落在 <Select> 上。已确认：
 * - ResizeObserver 已用 functional setState + 比较 prev bail-out 防反馈环
 * - filterOptions 已 useMemo 稳住
 * - Suspense 嵌套冲突已移除（dynamic ssr:false 自带 Suspense）
 * 仍然复现，所以定位到 antd 6 Select 内部 useLayoutEffect 与 React 19 协同的
 * 已知问题（[antd: compatible] 警告链路上 rc-select 的 setState sync）。
 *
 * 这里只用于节点类型筛选器这一处，过滤数据 ≤ 10 个、原生 <select> 完全胜任。
 * 画布内其它 antd 组件（Modal / Tag / Popconfirm / Input / Spin / Empty / App）
 * 不在报错栈里，保持不动。
 */
...
function NativeSelect({
  ...
  return (
    <select
      ...
      className={
        className ??
        "h-6 rounded border border-black/10 bg-transparent px-1 text-xs outline-none dark:border-white/15"
      }
      {...rest}
    >
      {options.map((option) => (
...
export function CanvasSidePanel({
  ...
  return (
    <motion.div
      ...
      transition={{
        duration: resizing ? 0 : PANEL_MOTION_SECONDS,
        ease: PANEL_EASE,
      }}
      style={{
        overflow: "clip",
        pointerEvents: panelClosing ? "none" : undefined,
      }}
    >
      <motion.aside
      ...
      >
        <div className="flex items-center gap-5 px-4 pt-3.5">
          <TabButton
            label={t("canvas.sidePanel.canvas")}
            active={tab === "canvas"}
            theme={theme}
        ...
        <button
          ...
          aria-label={t("canvas.sidePanel.resize")}
        />
      </motion.aside>
    </motion.div>
  );
...
function TabButton({
  ...
  return (
    ...
    >
      ...
      {active ? (
        <motion.span
          layoutId="sidePanelTabIndicator"
...
function CanvasNodesTab({
  ...
  const filtered = useMemo(() => {
    ...
    return nodes.filter(
      (node) =>
        ...
        (!query ||
          [node.title, node.metadata?.content, node.metadata?.prompt]
            .filter(Boolean)
            .join(" ")
            .toLowerCase()
            .includes(query))
  ...
  // antd 6 的 Select 内部对 options 引用稳定性敏感（rc-select 的 useLayoutEffect
  // 用 === 比较 value 来同步 selected state）。每次渲染 new array 会触发它
  // 内部的状态 sync → setState → re-render → sync → 死循环。
  // 用 useMemo + 稳定 t() 给 options 引用稳起来。
    ...
    () =>
      NODE_FILTER_VALUES.map((value) => ({
        ...
        label:
          value === "all"
            ? t("common.all")
            : t(`canvas.sidePanel.filter.${value}`),
      })),
    [t]
  );
  ...
  const allChecked =
    filtered.length > 0 && filtered.every((node) => checked.has(node.id));
  const toggleAll = () =>
    setChecked(
      allChecked ? new Set() : new Set(filtered.map((node) => node.id))
    );
  ...
  const handleExport = async () => {
    ...
    try {
      await exportCanvasNodes(
        targets,
        t("canvas.sidePanel.exportName", { count: targets.length })
      );
      message.success(
        t("canvas.sidePanel.exported", { count: targets.length })
      );
      exitSelect();
    } catch (error) {
      console.error(error);
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2 px-3 pb-2.5 pt-1">
        ...
        <button
          ...
          style={
            selectMode
              ? { color: theme.toolbar.activeText, opacity: 1 }
              : undefined
          }
        >
        ...
        {selectMode ? null : (
          <NativeSelect
            ...
            aria-label={t("canvas.sidePanel.filterLabel")}
          />
        )}
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
        {filtered.length ? (
            ...
            {filtered.map((node) => {
              ...
              return (
                <div
                  ...
                  style={
                    active ? { background: theme.toolbar.activeBg } : undefined
                  }
                >
                  <button
                    type="button"
                    ...
                    title={
                      selectMode ? undefined : t("canvas.sidePanel.focusNode")
                    }
                  >
                    {selectMode ? (
                    ...
                    <span className="min-w-0 flex-1 space-y-0.5">
                      <span className="block truncate text-sm font-medium leading-snug">
                        {node.title ||
                          getNodeDefinition(node.type)?.title ||
                          t("canvas.node.untitled")}
                      </span>
                      <span className="block truncate text-xs leading-snug opacity-50">
                    ...
                    node.metadata.status !== "idle" ? (
                      <span
                        ...
                        style={{
                          background:
                            STATUS_COLOR[node.metadata.status] || "transparent",
                        }}
                      />
                    ) : null}
                  </button>
          ...
          <div className="pt-16 text-center text-sm opacity-40">
...
function CheckMark({
  ...
  return (
    <span
      ...
      style={{
        borderColor: checked ? theme.toolbar.activeText : theme.node.stroke,
        background: checked ? theme.toolbar.activeText : "transparent",
      }}
    >
...
const CanvasAssetsTab = memo(function CanvasAssetsTab({
  onInsert,
  theme,
}: {
  ...
  const allTags = useMemo(
    () =>
      Array.from(new Set(assets.flatMap((asset) => asset.tags || []))).slice(
        0,
        20
      ),
    [assets]
  ...
  const groups = useMemo(
    () =>
      ASSET_GROUPS.map((group) => ({
        ...group,
        items: filtered.filter((asset) => asset.kind === group.kind),
...
function AssetCard({
  ...
  return (
    <div
      className="group relative aspect-square overflow-hidden rounded-xl border transition duration-200 hover:-translate-y-0.5 hover:shadow-lg"
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center gap-2.5 opacity-0 transition duration-200 group-hover:opacity-100">
        ...
        <Popconfirm
          title={t("canvas.sidePanel.removeAssetTitle")}
          okText={t("canvas.sidePanel.remove")}
...
function AssetCover({ asset }: { asset: Asset }) {
  if (asset.kind === "text")
    return (
      <div className="size-full overflow-hidden whitespace-pre-wrap break-words p-2.5 text-[11px] leading-snug opacity-80">
        {asset.data.content}
      </div>
    );
  if (asset.kind === "video") {
...
const CanvasPromptsTab = memo(function CanvasPromptsTab({
  onInsert,
  theme,
}: {
  onInsert: (payload: InsertAssetPayload) => void;
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
        <div className="space-y-1">
          {enabledSources.length ? (
            ...
            <Empty
              image={Empty.PRESENTED_IMAGE_SIMPLE}
              description={t("canvas.sidePanel.noPrompts")}
              className="pt-12"
            />
          )}
        </div>
        ...
        <div className="px-1 pb-2 pt-1">
          {query.isLoading ? (
            ...
            <button
              ...
              className="block w-full py-4 text-center text-xs text-red-500 opacity-80 transition hover:opacity-100"
            >
            ...
            <div className="space-y-1.5">
              {filtered.map((item) => (
                <PromptRow
                  ...
                  item={item}
                  theme={theme}
                  onInsert={() => insertPrompt(item)}
                  onView={() => onView(item)}
                />
              ))}
            </div>
          ) : (
            <div className="py-4 text-center text-xs opacity-40">
              {keyword.trim()
                ? t("canvas.sidePanel.noMatchingPrompts")
                : t("canvas.sidePanel.sourceEmpty")}
            </div>
          )}
        </div>
      ) : null}
  ...
  return (
    <div className="group relative flex items-center gap-2.5 rounded-lg px-2 py-2 transition hover:bg-black/5 dark:hover:bg-white/5">
      ...
      >
        ...
        <div className="mt-0.5 truncate text-xs leading-snug opacity-50">
          {item.prompt}
        </div>
      </button>
