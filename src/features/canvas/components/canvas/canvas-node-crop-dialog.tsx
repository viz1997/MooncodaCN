      ...
      setCrop(
        mode === "move"
          ...
          : resizeCrop(
              ...
              dy,
              handle || "se",
              resolveRatio(ratioPreset, image, fixedRatio),
              box
            )
      );
    };
      ...
      <div className="space-y-4">
        ...
        >
          <div className="relative" style={viewport.contentStyle}>
            <div
              ref={boxRef}
              className="absolute isolate overflow-hidden rounded-lg bg-black select-none [backface-visibility:hidden] [contain:layout_paint] [transform:translateZ(0)]"
            ...
            >
              ...
              >
                <img
                  src={dataUrl}
                  alt=""
                  className="block h-full w-full object-contain opacity-90"
                  draggable={false}
                />
              </div>
              <CropMask crop={crop} />
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border px-3 py-2">
          <div className="flex flex-wrap items-center gap-3 text-sm opacity-80">
            ...
            <span>
              {t("canvas.editors.ratio", {
                ratio: cropSize
                  ? formatRatio(cropSize.width, cropSize.height)
                  : t("canvas.editors.unknown"),
              })}
            </span>
            {image ? (
              <span>
                {t("canvas.editors.original", {
                  width: image.width,
                  height: image.height,
                })}
              </span>
          ...
          <Segmented
            ...
            onChange={(value) => {
              ...
              const currentRatio = image
                ? (crop.width * image.width) /
                  Math.max(1, crop.height * image.height)
                : null;
              ...
              if (ratio && image)
                setCrop((current) => fitCropToRatio(current, ratio, image));
            }}
          />
        </div>

        <div className="flex items-center justify-end gap-2">
          <Button onClick={() => setCrop(defaultCrop)}>
...
function resizeCrop(
  ...
  if (aspectRatio) {
    ...
    const useWidth =
      horizontalOnly ||
      (handle.length > 1 &&
        Math.abs(dx * box.width) >= Math.abs(dy * box.height));
  ...
  if (aspectRatio) {
    ...
    const scaleDown = Math.min(
      1,
      1 / Math.max(next.width, 0.001),
      1 / Math.max(next.height, 0.001)
    );
    next.width *= scaleDown;
    next.height *= scaleDown;
    if (next.width < minSize || next.height < minSize) {
      const minimumScale = Math.max(
        minSize / Math.max(next.width, 0.001),
        minSize / Math.max(next.height, 0.001)
      );
