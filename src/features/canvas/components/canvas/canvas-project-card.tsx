...
import {
  type CanvasProject,
  useCanvasStore,
} from "@/features/canvas/stores/canvas/use-canvas-store";
...
export function CanvasProjectCard({ project }: { project: CanvasProject }) {
  ...
  const setEditingTitle = useCanvasUiStore(
    (state) => state.setEditingProjectTitle
  );
  const stopEditing = useCanvasUiStore((state) => state.stopEditingProject);
  const toggleSelected = useCanvasUiStore(
    (state) => state.toggleSelectedProjectId
  );
  ...
  const open = () => {
    ...
    router.push(`/dashboard/canvas/${project.id}${query ? `?${query}` : ""}`);
  };
  const saveTitle = () => {
