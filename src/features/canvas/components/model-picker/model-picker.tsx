...
import {
  ...
  SelectItem,
  SelectTrigger,
} from "@/features/canvas/components/ui/select";
      ...
      onOpenChange={(nextOpen) => {
        ...
        if (nextOpen)
          window.dispatchEvent(
            new CustomEvent("model-picker-open", { detail: pickerId })
          );
        setOpen(nextOpen);
      }}
...
function emptyModelLabel(config: AiConfig, capability?: ModelCapability) {
  const label = capability
    ? i18n.t(`settingsPanels.model.capabilities.${capability}`)
    : "";
  if (capability && config.models.length)
    return i18n.t("settingsPanels.model.assign", { capability: label });
...
function resolveModelIcon(model: string) {
  ...
  if (name.includes("deepseek") || name.includes("deepseek"))
