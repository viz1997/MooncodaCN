...
import { fetchChannelModels } from "@/features/canvas/services/api/image";
import type { ModelChannel } from "@/features/canvas/stores/use-config-store";
...
export function ModelSelectModal({
  ...
  const visibleList = useMemo(() => {
    ...
    return keyword
      ? currentList.filter((name) => name.toLowerCase().includes(keyword))
      : currentList;
  }, [currentList, search]);
  const visibleSelectedCount = visibleList.filter((name) =>
    selected.has(name)
  ).length;
  ...
  const addManual = () => {
    ...
    if (!name) return;
    if (!fetched.includes(name) && !existing.includes(name))
      setFetched((current) => [name, ...current]);
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      setActiveTab("new");
      message.success(
        t("config.modelSelect.fetched", { count: models.length })
      );
    } catch (error) {
      message.error(
        error instanceof Error
          ? error.message
          : t("config.modelSelect.fetchFailed")
      );
    } finally {
      setLoading(false);
    }
  ...
  const confirm = () => {
    const ordered = [...existing, ...fetched]
  ...
  return (
    <Modal
      ...
      title={
        <span>
          {t("config.modelSelect.title")}{" "}
          <span className="ml-2 text-xs font-normal text-stone-500">
            {t("config.modelSelect.selected", {
              selected: selected.size,
              total: new Set([...existing, ...fetched]).size,
            })}
          </span>
        </span>
      }
    ...
    >
      <div className="flex flex-wrap items-center gap-3">
        <Input
          ...
          placeholder={t("config.modelSelect.search")}
        ...
        <Input
          ...
          placeholder={t("config.modelSelect.modelName")}
        />
      ...
      <Tabs
        ...
        items={[
          {
            key: "new",
            label: t("config.modelSelect.fetchedTab", {
              count: fetched.length,
            }),
          },
          {
            key: "existing",
            label: t("config.modelSelect.existingTab", {
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <span className="text-xs text-stone-500">
          {t("config.modelSelect.visibleSelected", {
      ...
      {visibleList.length ? (
        ...
        <div className="py-8 text-center text-sm text-stone-500">
          {t(
            activeTab === "new"
              ? "config.modelSelect.fetchedEmpty"
              : "config.modelSelect.existingEmpty"
          )}
        </div>
      )}
    </Modal>
