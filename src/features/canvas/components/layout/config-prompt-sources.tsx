...
import type { PromptSource } from "@/features/canvas/services/api/prompt-source-presets";
import {
  fetchPromptSourceStatuses,
  refreshAllSources,
  refreshSource,
} from "@/features/canvas/services/api/prompts";
import {
  PROMPT_SOURCE_INTERVALS,
  usePromptSourceStore,
} from "@/features/canvas/stores/use-prompt-source-store";
...
export function ConfigPromptSources() {
  ...
  const statusQuery = useQuery({
    ...
    queryFn: fetchPromptSourceStatuses,
  });
  ...
  const intervalOptions = PROMPT_SOURCE_INTERVALS.map((value) => ({
    value,
    label: t(`config.promptSources.intervals.${intervalKey(value)}`),
  }));
  ...
  const handleRefreshOne = async (source: PromptSource) => {
    ...
    try {
      ...
      await invalidatePrompts();
      message.success(
        t("config.promptSources.refreshed", {
          name: source.name,
          count: result.count,
        })
      );
      ...
      message.error(
        error instanceof Error
          ? error.message
          : t("config.promptSources.refreshFailedCached")
      );
    } finally {
      setRefreshingId("");
    }
  ...
  const handleRefreshAll = async () => {
    ...
    try {
      ...
      if (result.failureCount)
        message.warning(
          t("config.promptSources.refreshPartial", {
        ...
        message.success(
          t("config.promptSources.refreshAllSuccess", {
            sources: result.successCount,
            total: result.total,
          })
        );
    } catch (error) {
      message.error(
        error instanceof Error
          ? error.message
          : t("config.promptSources.refreshFailed")
      );
    } finally {
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-end gap-3">
        <Button
          ...
          onClick={() => setEditingSource(addSource())}
        >
      ...
      <div className="space-y-2">
        {sources.map((source) => {
          ...
          return (
            <div
              key={source.id}
              className="flex flex-wrap items-center gap-3 rounded-lg border border-stone-200 px-4 py-3 dark:border-stone-800"
            >
              <Switch
                size="small"
                ...
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  {source.builtIn ? (
                    <Tag className="m-0 shrink-0 text-[10px]">
                      {t("config.promptSources.builtIn")}
                    </Tag>
                  ) : null}
                </div>
                <div className="mt-1 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-xs text-stone-500">
                  <a
                    className="max-w-full truncate hover:text-stone-800 hover:underline dark:hover:text-stone-200"
                  ...
                  <span className="tabular-nums">
                    {t("config.promptSources.itemCount", {
                  ...
                  {status?.lastError ? (
                    ...
                    <Tag className="m-0 text-[10px]">
                      {t("config.promptSources.unsynced")}
                    </Tag>
                  )}
                  <span>
                    {status?.lastSuccessAt
                      ? t("config.promptSources.lastSuccess", {
                          time: formatTime(
                            status.lastSuccessAt,
                            i18n.resolvedLanguage
                          ),
                        })
                      : t("config.promptSources.neverFetched")}
                  </span>
                </div>
              </div>
              <div className="ml-auto flex flex-wrap justify-end gap-2">
                ...
                <Button
                  ...
                  onClick={() => void handleRefreshOne(source)}
                >
      ...
      <section className="mt-5 rounded-lg border border-stone-200 p-4 dark:border-stone-800">
        <div className="mb-3 text-sm font-semibold">
          {t("config.promptSources.schedule")}
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <span className="text-xs text-stone-500">
              {t("config.promptSources.interval")}
            </span>
            <Select
              size="small"
              className="w-36"
              ...
              onChange={(value) => updateSchedule("intervalMinutes", value)}
            />
          </div>
          <Button
            size="small"
          ...
          <span className="text-xs text-stone-500">
            {schedule.lastFetchedAt
              ? t("config.promptSources.lastFetched", {
                  time: formatTime(
                    schedule.lastFetchedAt,
                    i18n.resolvedLanguage
                  ),
                })
              : t("config.promptSources.neverScheduled")}
          </span>
        </div>
        <div className="mt-2 text-xs text-stone-400">
          {t("config.promptSources.scheduleDescription")}
        </div>
      </section>

      <PromptSourceEditorDrawer
        open={Boolean(editingSource)}
        source={editingSource}
        onSave={handleSave}
        onClose={() => setEditingSource(null)}
      />
      <PromptSourceContentModal
