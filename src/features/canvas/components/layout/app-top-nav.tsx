...
import { MobileNavDrawer } from "@/features/canvas/components/layout/mobile-nav-drawer";
import { UserStatusActions } from "@/features/canvas/components/layout/user-status-actions";
import {
  type NavigationToolSlug,
  navigationTools,
} from "@/features/canvas/constant/navigation-tools";
...
export function AppTopNav() {
  ...
  // next-intl 的 usePathname 已剥掉 /zh /en 前缀，所以 URL /zh/dashboard/canvas/abc
  // 在这里拿到的是 /dashboard/canvas/abc。编辑器需要隐藏站点头部。
  const hideHeader = /^\/dashboard\/canvas\/[^/]+/.test(pathname);
        ...
        <header className="sticky top-0 z-20 h-14 shrink-0 border-b border-stone-200 bg-background/90 backdrop-blur-xl dark:border-stone-800">
          <div className="mx-auto flex h-full max-w-7xl items-stretch justify-between gap-5 px-6">
            ...
            <div className="my-auto flex h-9 min-w-0 items-center justify-end gap-2 justify-self-end whitespace-nowrap">
              <Tooltip
                title={t(panelOpen ? "topNav.closeAgent" : "topNav.openAgent")}
              >
                <Button
                  type="text"
                  shape="circle"
                  ...
                  onClick={togglePanel}
                  aria-label={t(
                    panelOpen ? "topNav.closeAgent" : "topNav.openAgent"
                  )}
                />
              </Tooltip>
              <UserStatusActions />
