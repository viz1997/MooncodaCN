...
import { ChannelEditorDrawer } from "@/features/canvas/components/layout/channel-editor-drawer";
import { ConfigLocalStorage } from "@/features/canvas/components/layout/config-local-storage";
import { ConfigPromptSources } from "@/features/canvas/components/layout/config-prompt-sources";
import { ModelPicker } from "@/features/canvas/components/model-picker/model-picker";
import type { AppLocale } from "@/features/canvas/i18n";
import {
  audioFormatOptions,
  audioVoiceOptions,
...
import {
  ...
  syncAppDataToWebdav,
} from "@/features/canvas/services/app-sync";
import {
  exportAppConfig,
  importAppConfig,
} from "@/features/canvas/services/config-file";
import {
  testWebdavConnection,
  WEBDAV_MANIFEST_FILE_NAME,
} from "@/features/canvas/services/webdav-sync";
import {
  type AiConfig,
  type ApiCallFormat,
  type ConfigTabKey,
...
const modelGroups: ModelGroup[] = [
  {
    capability: "image",
    modelKey: "imageModel",
    labelKey: "config.preferences.defaultImageModel",
  },
  {
    capability: "video",
    modelKey: "videoModel",
    labelKey: "config.preferences.defaultVideoModel",
  },
  {
    capability: "text",
    modelKey: "textModel",
    labelKey: "config.preferences.defaultTextModel",
  },
  {
    capability: "audio",
    modelKey: "audioModel",
    labelKey: "config.preferences.defaultAudioModel",
  },
];

const webdavDomainKeys: AppSyncDomainKey[] = [
  "canvas",
  "assets",
  "image-workbench",
  "video-workbench",
];
function createWebdavDomainProgress(): Record<
  AppSyncDomainKey,
  WebdavDomainProgress
> {
...
export function AppConfigPanel({
  ...
  const [webdavDomainProgress, setWebdavDomainProgress] = useState(
    createWebdavDomainProgress
  );
  const config = useConfigStore((state) => state.config);
  const webdav = useConfigStore((state) => state.webdav);
  const updateConfig = useConfigStore((state) => state.updateConfig);
  const updateWebdavConfig = useConfigStore(
    (state) => state.updateWebdavConfig
  );
  ...
  const editingChannel =
    config.channels.find((channel) => channel.id === editingChannelId) || null;
  ...
  const finishConfig = () => {
    const ready = config.channels.some(
      (channel) =>
        channel.baseUrl.trim() && channel.apiKey.trim() && channel.models.length
    );
    setConfigDialogOpen(false);
    if (!ready) return;
    message.success(
      t(shouldPromptContinue ? "config.savedContinue" : "config.saved")
    );
    clearPromptContinue();
  };
  ...
  const loadConfigFile = async (file: File) => {
    try {
      ...
      message.error(
        error instanceof Error ? error.message : t("config.importFailed")
      );
    } finally {
  ...
  const addChannel = () => {
    const channel = createModelChannel({
      name: t("config.channels.numberedName", {
        count: config.channels.length + 1,
      }),
    });
  ...
  const testWebdav = async () => {
    ...
    try {
      ...
      message.error(
        error instanceof Error ? error.message : t("config.webdav.testFailed")
      );
    } finally {
  ...
  const syncWebdav = async () => {
    ...
    try {
      ...
      message.success(
        t("config.webdav.completed", {
          projects: result.projects,
          assets: result.assets,
          records: result.imageLogs + result.videoLogs,
          files: result.uploadedFiles,
          bytes: formatBytes(result.uploadedBytes),
        })
      );
    } catch (error) {
      setWebdavSyncStatus(
        error instanceof Error ? error.message : t("config.webdav.failed")
      );
      message.error(
        error instanceof Error ? error.message : t("config.webdav.failed")
      );
        ...
        items={[
          {
            ...
            children: (
              <div>
                {/* Phase 3：内置 / 自定义 渠道模式切换 */}
                <div className="mb-4 rounded-xl border border-stone-200 bg-stone-50/60 p-4 dark:border-stone-800 dark:bg-stone-900/40">
                  <div className="mb-2 text-sm font-medium">
                    {t("config.channelMode.title")}
                  </div>
                  <div className="mb-3 text-xs text-stone-500">
                    {t("config.channelMode.description")}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button
                      type={
                        config.channelMode === "remote" ? "primary" : "default"
                      }
                      onClick={() => updateConfig("channelMode", "remote")}
                    >
                      {t("config.channelMode.remote")}
                    </Button>
                    <Button
                      type={
                        config.channelMode === "local" ? "primary" : "default"
                      }
                      onClick={() => updateConfig("channelMode", "local")}
                    >
                      {t("config.channelMode.local")}
                    </Button>
                  </div>
                  {config.channelMode === "remote" ? (
                    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
                      {t("config.channelMode.remoteHint")}
                    </div>
                  ) : null}
                </div>
                ...
                {config.channelMode === "local" ? (
                        ...
                        >
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 truncate text-xs text-stone-500">
                              {apiFormatLabel(channel.apiFormat, t)} ·{" "}
                              {t("config.channels.modelCount", {
                                count: channel.models.length,
                              })}{" "}
                              ·{" "}
                              {channel.baseUrl ||
                                t("config.channels.missingUrl")}
                            </div>
                          </div>
                  ...
                  <div className="space-y-3">
                    <div className="text-xs text-stone-500">
                      {t("config.builtin.description")}
                    </div>
                    <div className="grid gap-3 md:grid-cols-2">
                      <div className="rounded-xl border border-stone-200 p-4 dark:border-stone-800">
                        <div className="mb-1 text-sm font-semibold">
                          {t("config.builtin.imageLabel")}
                        </div>
                        <div className="text-xs text-stone-500">
                          {t("config.builtin.imageHint")}
                        </div>
                        <div className="mt-2 flex flex-wrap gap-1.5 text-xs">
                          <Tag color="blue">{t("config.builtin.cost20")}</Tag>
                          <Tag color="purple">{t("config.builtin.cost25")}</Tag>
                        </div>
                      </div>
                      <div className="rounded-xl border border-stone-200 p-4 dark:border-stone-800">
                        <div className="mb-1 text-sm font-semibold">
                          {t("config.builtin.audioLabel")}
                        </div>
                        <div className="text-xs text-stone-500">
                          {t("config.builtin.audioHint")}
                        </div>
                        <div className="mt-2 text-xs">
                          <Tag color="blue">{t("config.builtin.cost10")}</Tag>
                        </div>
                      </div>
                      <div className="rounded-xl border border-stone-200 p-4 dark:border-stone-800">
                        <div className="mb-1 text-sm font-semibold">
                          {t("config.builtin.textLabel")}
                        </div>
                        <div className="text-xs text-stone-500">
                          {t("config.builtin.textHint")}
                        </div>
                        <div className="mt-2 text-xs">
                          <Tag color="blue">{t("config.builtin.cost5")}</Tag>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            children: (
              <Form layout="vertical" requiredMark={false}>
                ...
                <div className="grid gap-4 md:grid-cols-4">
                  <Form.Item
                    label={t("config.preferences.canvasImageCount")}
                    extra={t("config.preferences.canvasImageCountDescription")}
                    className="mb-4"
                  >
                    <Input
                      type="number"
                      ...
                      onChange={(event) =>
                        updateConfig("canvasImageCount", event.target.value)
                      }
                      onBlur={(event) =>
                        updateConfig(
                          "canvasImageCount",
                          normalizeImageCount(event.target.value)
                        )
                      }
                    />
                  </Form.Item>
                  <Form.Item
                    label={t("config.preferences.audioVoice")}
                    className="mb-4"
                  >
                    <Select
                  ...
                  <Form.Item
                    label={t("config.preferences.audioFormat")}
                    className="mb-4"
                  >
                    <Select
                  ...
                  <Form.Item
                    label={t("config.preferences.audioSpeed")}
                    className="mb-4"
                  >
                    <Input
                      type="number"
                      ...
                      onChange={(event) =>
                        updateConfig("audioSpeed", event.target.value)
                      }
                      onBlur={(event) =>
                        updateConfig(
                          "audioSpeed",
                          normalizeAudioSpeedValue(event.target.value)
                        )
                      }
                    />
                  </Form.Item>
                ...
                <Form.Item
                  label={t("config.preferences.audioInstructions")}
                  className="mb-4"
                >
                  <Input.TextArea
                    rows={2}
                    ...
                    placeholder={t(
                      "config.preferences.audioInstructionsPlaceholder"
                    )}
                    onChange={(event) =>
                      updateConfig("audioInstructions", event.target.value)
                    }
                  />
                </Form.Item>
                <Form.Item
                  label={t("config.preferences.systemPrompt")}
                  className="mb-0"
                >
                  <Input.TextArea
                    rows={4}
                    ...
                    placeholder={t(
                      "config.preferences.systemPromptPlaceholder"
                    )}
                    onChange={(event) =>
                ...
                <section className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                    ...
                    <div className="text-xs text-stone-500">
                      {webdav.lastSyncedAt
                        ? t("config.webdav.lastSynced", {
                            time: formatWebdavTime(webdav.lastSyncedAt, locale),
                          })
                        : t("config.webdav.neverSynced")}
                    </div>
                  </div>
                  <div className="grid gap-4 md:grid-cols-2">
                    <Form.Item label={t("config.webdav.url")} className="mb-4">
                      <Input
                        ...
                        onChange={(event) =>
                          updateWebdavConfig("url", event.target.value)
                        }
                      />
                    </Form.Item>
                    <Form.Item
                      label={t("config.webdav.directory")}
                      extra={t("config.webdav.directoryDescription", {
                        manifest: WEBDAV_MANIFEST_FILE_NAME,
                      })}
                      className="mb-4"
                    >
                      <Input
                        value={webdav.directory}
                        placeholder="infinite-canvas"
                        onChange={(event) =>
                          updateWebdavConfig("directory", event.target.value)
                        }
                      />
                    </Form.Item>
                    <Form.Item
                      label={t("config.webdav.username")}
                      className="mb-0"
                    >
                      <Input
                    ...
                    <Form.Item
                      label={t("config.webdav.password")}
                      className="mb-0"
                    >
                      <Input.Password
                        ...
                        onChange={(event) =>
                          updateWebdavConfig("password", event.target.value)
                        }
                      />
                    </Form.Item>
                  </div>
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    <Button
                      ...
                      disabled={!webdavReady || syncingWebdav}
                    ...
                    <Button
                      ...
                      disabled={!webdavReady || testingWebdav}
                      loading={syncingWebdav}
                      onClick={() => void syncWebdav()}
                    >
                      {t(
                        syncingWebdav
                          ? "config.webdav.syncing"
                          : "config.webdav.syncNow"
                      )}
                    </Button>
                    {webdavSyncStatus ? (
                      <span className="text-xs text-stone-500">
                        {syncStageLabel(webdavSyncStatus, t)}
                      </span>
                    ) : null}
                  </div>
                  {syncingWebdav || webdavSyncStatus ? (
                    <WebdavProgressGrid progress={webdavDomainProgress} t={t} />
                  ) : null}
                </section>
              </Form>
            ),
          },
          {
            ...
            children: (
              <ConfigLocalStorage active={activeTab === "local-storage"} />
            ),
          },
        ]}
      />
...
export function AppConfigModal() {
  ...
  return (
    <Modal
      ...
      styles={{
        body: { maxHeight: "72vh", overflowY: "auto", paddingRight: 12 },
      }}
      footer={null}
    >
...
function normalizeImageCount(value: string) {
  return String(
    Math.max(1, Math.min(15, Math.floor(Math.abs(Number(value)) || 3)))
  );
}
...
function WebdavProgressGrid({
  progress,
  t,
}: {
  progress: Record<AppSyncDomainKey, WebdavDomainProgress>;
  t: TFunction;
}) {
  return (
          ...
          >
            <div className="mb-1 flex min-w-0 items-center justify-between gap-3 text-xs">
              <span className="shrink-0 font-medium text-stone-700 dark:text-stone-200">
                {t(`config.webdav.domains.${domainTranslationKey(key)}`)}
              </span>
            ...
            <Progress
              percent={getWebdavProgressPercent(item)}
              size="small"
...
function syncStageLabel(stage: string, t: TFunction) {
  if (stage === "等待本地数据加载")
    return t("config.webdav.stages.localWaiting");
  ...
  if (stage === "检查缺失媒体")
    return t("config.webdav.stages.checkMissingMedia");
  if (stage === "下载媒体") return t("config.webdav.stages.downloadMediaFile");
  if (stage === "检查本地媒体")
    return t("config.webdav.stages.checkLocalMedia");
  if (stage.startsWith("上传媒体 "))
    return t("config.webdav.stages.uploadMediaFile", { size: stage.slice(5) });
  if (stage === "完成") return t("config.webdav.stages.complete");
  if (stage.startsWith("上传清单 "))
    return t("config.webdav.stages.uploadManifest", { size: stage.slice(5) });
  return stage;
}
...
function getWebdavProgressStatus(
  item: WebdavDomainProgress
): "normal" | "active" | "success" | "exception" {
  if (item.status === "success" || item.status === "exception")
    return item.status;
