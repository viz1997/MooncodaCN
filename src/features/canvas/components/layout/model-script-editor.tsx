...
import {
  ...
  getPluginTemplates,
  getPluginVariables,
} from "@/features/canvas/services/api/model-plugin";
import type { ModelCapability } from "@/features/canvas/stores/use-config-store";
...
export function ModelScriptEditor({
  open,
  capability,
  modelName,
  value,
  ...
  const variables = getPluginVariables().filter(
    (variable) =>
      !variable.capabilities || variable.capabilities.includes(capability)
  );
      ...
      title={
        <div>
          ...
          <div className="mt-1 text-xs font-normal text-stone-500">
            {t("config.scriptEditor.description")}
          </div>
        </div>
      }
      width={1080}
          ...
          <div className="flex flex-wrap items-center gap-2">
            {templates.map((template) => (
              <Button
                key={template.label}
                size="small"
                onClick={() => setDraft(template.script)}
              >
                {t("config.scriptEditor.insertTemplate", {
                  name: template.label,
                })}
              </Button>
            ))}
        ...
        <aside className="flex w-[320px] shrink-0 flex-col overflow-y-auto border-r border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-stone-900/40">
          <div className="border-b border-stone-200/70 px-4 py-3 dark:border-stone-800/70">
            <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-stone-400">
              {t("config.scriptEditor.returnRequirements")}
            </div>
            <div className="text-xs leading-6 text-stone-600 dark:text-stone-300">
          ...
          <div className="px-4 py-3">
            <div className="mb-2.5 flex items-center justify-between">
              <span className="text-[11px] font-semibold uppercase tracking-wide text-stone-400">
                {t("config.scriptEditor.variables")}
              </span>
              <span className="text-[10px] text-stone-400">
                {t("config.scriptEditor.insert")}
              </span>
            </div>
