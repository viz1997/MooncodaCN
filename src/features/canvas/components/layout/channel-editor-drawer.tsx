...
export function ChannelEditorDrawer({
  ...
  const changeApiFormat = (apiFormat: ApiCallFormat) => {
    const baseUrl =
      !draft.baseUrl.trim() ||
      draft.baseUrl.trim() === defaultBaseUrlForApiFormat(draft.apiFormat)
  ...
  const applySelection = (names: string[]) => {
    ...
    setModels(
      names.map(
        (name) => map.get(name) || { name, capability: guessCapability(name) }
      )
    );
  };
  ...
  const setCapability = (name: string, capability: ModelCapability) =>
    setModels(
      draft.models.map((model) =>
        model.name === name ? { ...model, capability } : model
      )
    );
  const setScript = (name: string, script: string) =>
    setModels(
      draft.models.map((model) =>
        model.name === name ? { ...model, script: script || undefined } : model
      )
    );
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="block">
          <span className="mb-1 block text-sm font-medium">
            {t("config.channelEditor.protocol")}
          </span>
          <Select
            className="w-full"
            value={draft.apiFormat}
            options={apiFormatOptions}
            onChange={changeApiFormat}
          />
        </label>
        <label className="block md:col-span-2">
          <span className="mb-1 block text-sm font-medium">
            {t("config.channelEditor.baseUrl")}
          </span>
          <Input
            value={draft.baseUrl}
      ...
      <div className="mt-6 mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <div className="mt-0.5 text-xs text-stone-500">
            {t("config.channelEditor.modelDescription", {
      ...
      <div className="space-y-2 rounded-lg border border-stone-200 p-2 dark:border-stone-800">
        {draft.models.length ? (
          draft.models.map((model) => (
            <div
              key={model.name}
              className="flex flex-wrap items-center gap-3 rounded-md px-2 py-1.5 hover:bg-stone-50 dark:hover:bg-stone-900/40"
            >
              <span
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                >
                  {t(
                    model.script
                      ? "config.channelEditor.scriptReady"
                      : "config.channelEditor.script"
                  )}
                </Button>
                <Button
                  size="small"
      ...
      <ModelScriptEditor
        ...
        onSave={(script) =>
          scriptTarget && setScript(scriptTarget.name, script)
        }
